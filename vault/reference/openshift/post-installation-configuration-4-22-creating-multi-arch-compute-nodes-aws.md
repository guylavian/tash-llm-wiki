...
type: reference
domain: openshift
slug: post-installation-configuration-4-22-creating-multi-arch-compute-nodes-aws
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/post_installation_configuration/creating-multi-arch-compute-nodes-aws
version: 4.22
...
+
.Example `MachineSet` object for an {aws-short} 64-bit ARM or x86 compute node
[source,yaml]
----
...
----
+
`<role>-<zone>`:: Specifies the infrastructure ID, role node label, and zone.
+
`<role>`:: Specifies the role node label to add.
+
`ami.id`:: Specifies a {op-system-first} Amazon Machine Image (AMI) for your AWS region for the nodes. The {op-system} AMI must be compatible with the machine architecture.
+
[source,terminal]
----
...
----
+
`instanceType`:: Specifies a machine type that aligns with the CPU architecture of the chosen AMI. For more information, see "Tested instance types for AWS 64-bit ARM".
+
`availabilityZone`:: Specifies the zone. For example, `us-east-1a`. Ensure that the zone you select has machines with the required architecture.
+
`region`:: Specifies the region. For example, `us-east-1`. Ensure that the zone you select has machines with the required architecture.
