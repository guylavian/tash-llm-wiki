...
type: reference
domain: openshift
slug: security-4-22-file-integrity-operator-release-notes
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/security/file-integrity-operator-release-notes
version: 4.22
family: security
...
[role="_abstract"]
The File Integrity Operator for OpenShift Container Platform continually runs file integrity checks on {op-system} nodes.
...
In FIO 1.3.1, increasing nodes in {ibm-z-name} clusters might result in `Failed` File Integrity node status. For more information, see Adding nodes in {ibm-power-name} clusters can result in failed File Integrity node status.
