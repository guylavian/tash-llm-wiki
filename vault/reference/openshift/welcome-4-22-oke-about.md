...
type: reference
domain: openshift
slug: welcome-4-22-oke-about
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/welcome/oke_about
version: 4.22
family: welcome
...
launching containers. You download and install {oke} the same way as OpenShift Container Platform
...
.Product comparison for {oke} and OpenShift Container Platform
|===
2+| |{oke} |OpenShift Container Platform
...
vulnerabilities and errors protection as OpenShift Container Platform. {oke} includes a
...
{oke} uses the same security options and default settings as the OpenShift Container Platform.
...
OpenShift Container Platform offers are available in {oke}. {oke} offers full access to the
...
{oke} uses the same installation and upgrade automation as OpenShift Container Platform.
...
OpenShift Container Platform. {oke} allows customers to use a Kubernetes Network Policy to
...
Helm, and OpenShift Container Platform templates. {oke} users can use both the `oc` and
...
With an {oke} subscription, you receive access to the OpenShift Container Platform
...
{oke} does not include OpenShift Serverless support. Use OpenShift Container Platform
for this support.
...
OpenShift Container Platform are not supported in {oke}.

=== Advanced networking
The standard networking solutions in OpenShift Container Platform are supported with an
{oke} subscription. The OpenShift Container Platform Kubernetes CNI plugin for automation of
...
found in OpenShift Container Platform. That ingress solution is supported in {oke}.
...
=== {osc}
{oke} does not include {osc}. Use OpenShift Container Platform for this support.
...
The following table is a summary of the feature availability in {oke} and OpenShift Container Platform. Where applicable, it includes the name of the Operator that enables a feature.

.Features in {oke} and OpenShift Container Platform
[%header, cols="h,,,"]
|===
| Feature | {oke} | OpenShift Container Platform | Operator name
...
s| Feature s| {oke} s| OpenShift Container Platform s| Operator name
...
| RHT and {ibm-name} middleware à la carte purchases (not included in OpenShift Container Platform or {oke}) | Included | Included | N/A
| ISV or Partner Operator and Container Compatibility (not included in OpenShift Container Platform or {oke}) | Included | Included | N/A
...
{oke} is a subscription offering that provides OpenShift Container Platform with a limited set
of supported features at a lower list price. {oke} and OpenShift Container Platform are the
same product and, therefore, all software and features are delivered in both.
There is only one download, OpenShift Container Platform. {oke} uses the OpenShift Container Platform
