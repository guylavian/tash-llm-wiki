...
type: reference
domain: openshift
slug: networking-4-22-metallb-troubleshoot-support
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/networking/metallb-troubleshoot-support
version: 4.22
family: networking
...
The BFD implementation that Red{nbsp}Hat supports uses `FRRouting` (FRR) in a container that exists in an `frr-k8s` pod in the `openshift-frr-k8s` namespace.
...
You can determine the node name for the FRR-K8s pod with a command such as `oc get pods -n openshift-frr-k8s <frr_k8s_pod_name> -o jsonpath='{.spec.nodeName}'`.
