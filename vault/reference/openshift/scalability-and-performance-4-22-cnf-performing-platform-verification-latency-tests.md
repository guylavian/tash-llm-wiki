...
type: reference
domain: openshift
slug: scalability-and-performance-4-22-cnf-performing-platform-verification-latency-tests
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/scalability_and_performance/cnf-performing-platform-verification-latency-tests
version: 4.22
...
The `cnf-tests` container image is available at `registry.redhat.io/openshift4/cnf-tests-rhel9:v`.
...
registry.redhat.io/openshift4/cnf-tests-rhel9:v /usr/bin/test-run.sh \
...
registry.redhat.io/openshift4/cnf-tests-rhel9:v \
...
* Setting firmware parameters for system tuning
...
-e KUBECONFIG=/kubeconfig/kubeconfig registry.redhat.io/openshift4/cnf-tests-rhel9:v \
...
[role="_abstract"]
To validate node tuning and identify performance delays, run latency tests on your {sno} clusters. Evaluating these metrics ensures your environment is optimized for high-performance workloads.
...
----
$ podman run -v $(pwd)/:/kubeconfig:Z -e KUBECONFIG=/kubeconfig/kubeconfig \
-e LATENCY_TEST_RUNTIME=<time_in_seconds> registry.redhat.io/openshift4/cnf-tests-rhel9:v \
...
-e CNF_TESTS_IMAGE="cnf-tests-rhel9:v" \
-e LATENCY_TEST_RUNTIME=<time_in_seconds> \
<disconnected_registry>/cnf-tests-rhel9:v /usr/bin/test-run.sh --ginkgo.v --ginkgo.timeout="24h"
----
...
-e LATENCY_TEST_RUNTIME=<time_in_seconds> \
registry.redhat.io/openshift4/cnf-tests-rhel9:v /usr/bin/test-run.sh --ginkgo.v --ginkgo.timeout="24h"
----
+
where:
+
...
[role="_abstract"]
To make container images locally available for your deployment, mirror them to the built-in {product-registry}. This integrated component runs as a standard workload on your OpenShift Container Platform cluster to ensure continuous access to required files.
...
----
$ podman run -v $(pwd)/:/kubeconfig:Z -e KUBECONFIG=/kubeconfig/kubeconfig \
registry.redhat.io/openshift4/cnf-tests-rhel9:v /usr/bin/mirror \
