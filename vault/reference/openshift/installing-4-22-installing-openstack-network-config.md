...
type: reference
domain: openshift
slug: installing-4-22-installing-openstack-network-config
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/installing/installing-openstack-network-config
version: 4.22
family: installing
...
You can configure network settings for an OpenShift Container Platform on {rh-openstack-first} cluster after installation.
...
After you install OpenShift Container Platform, configure {rh-openstack-first} to allow application network traffic.
...
* Floating IP addresses are enabled as described in the OpenShift Container Platform on {rh-openstack} installation documentation.
...
For clusters that run on {rh-openstack-first}, you can enable Open vSwitch (OVS) hardware offloading.
