...
type: reference
domain: openshift
slug: installing-4-22-install-ibm-cloud-prerequisites
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/installing/install-ibm-cloud-prerequisites
version: 4.22
family: installing
...
You can use installer-provisioned installation to install OpenShift Container Platform on {ibm-cloud-bm} nodes. This document describes the prerequisites and procedures when installing OpenShift Container Platform on {ibm-cloud-name} nodes.
...
Before starting an installer-provisioned installation of OpenShift Container Platform on {ibm-cloud-bm}, address the following prerequisites and requirements.
...
To deploy an OpenShift Container Platform cluster on {ibm-cloud-bm} infrastructure, you must first provision the {ibm-cloud-name} nodes.
...
All nodes in the OpenShift Container Platform cluster must run in the same {ibm-cloud-name} data center.
...
Clients access the OpenShift Container Platform cluster nodes over the `baremetal` network. Configure {ibm-cloud-name} subdomains or subzones where the canonical name extension is the cluster name.
...
{ibm-cloud-bm} does not run DHCP on the public or private VLANs. After provisioning {ibm-cloud-name} nodes, you must set up a DHCP server for the public VLAN, which corresponds to OpenShift Container Platform's `baremetal` network.
...
See Installing the stand-alone {ibm-cloud-name} CLI for details on installing the {ibm-cloud-name} CLI.
