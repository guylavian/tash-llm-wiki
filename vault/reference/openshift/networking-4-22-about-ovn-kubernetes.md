...
type: reference
domain: openshift
slug: networking-4-22-about-ovn-kubernetes
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/networking/about-ovn-kubernetes
version: 4.22
family: networking
...
Part of {openshift-networking}, the OVN-Kubernetes network plugin is the default network provider for OpenShift Container Platform. OVN-Kubernetes is based on Open Virtual Network (OVN) and provides an overlay-based networking implementation. A cluster that uses the OVN-Kubernetes plugin also runs Open vSwitch (OVS) on each node. OVN configures OVS on each node to implement the declared network configuration.
...
* Session stickiness timeout in Kubernetes

[role="_additional-resources"]
.Additional resources

* Open Virtual Network website
