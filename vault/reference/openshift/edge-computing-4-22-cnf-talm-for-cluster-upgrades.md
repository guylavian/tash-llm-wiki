---
title: "Updating managed clusters with the {cgu-operator-full}"
type: reference
domain: openshift
slug: edge-computing-4-22-cnf-talm-for-cluster-upgrades
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/edge_computing/cnf-talm-for-cluster-upgrades
version: 4.22
...
[role="_abstract"]
The {cgu-operator-first} manages the deployment of {rh-rhacm-first} policies for one or more OpenShift Container Platform clusters. Using {cgu-operator} in a large network of clusters allows the phased rollout of policies to the clusters in limited batches. This helps to minimize possible service disruptions when updating. With {cgu-operator}, you can control the following actions:
...
{cgu-operator} supports the orchestration of the OpenShift Container Platform y-stream and z-stream updates, and day-two operations on y-streams and z-streams.
...
For more information about managed policies, see Policy Overview in the {rh-rhacm} documentation.
...
[role="_abstract"]
You can use the OpenShift Container Platform web console to install the {cgu-operator-full}.
...
topology-aware-lifecycle-manager..x   Topology Aware Lifecycle Manager   .x                                      Succeeded
----
...
{cgu-operator} compares the available disk space with the estimated OpenShift Container Platform image size to ensure that there is enough space. If a cluster has insufficient space, {cgu-operator} cancels pre-caching for that cluster and does not remediate policies on it.
...
For more information about {rh-rhacm} policies, see Policy overview.
...
version   4..5     True        True          43s     Working towards 4..7: 71 of 735 done (9% complete)
----
...
[role="_abstract"]
The {cgu-operator-first} is an OpenShift Container Platform Operator that remediates {rh-rhacm} policies. When issues occur, use the `oc adm must-gather` command to gather details and logs and to take steps in debugging the issues.
...
** Are the {rh-rhacm} and the OpenShift Container Platform versions compatible?
...
+
For more information on placements in {rh-rhacm}, see Placement overview.
+
For more information on tolerations in {rh-rhacm}, see Placing managed clusters by using taints and tolerations.
...
* Updating managed policies with {cgu-operator-full}
