---
title: "Installing a cluster on {oci-distributed-no-rt} by using the Agent-based Installer"
type: reference
domain: openshift
slug: installing-4-22-installing-oci-agent-based-installer
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/installing/installing-oci-agent-based-installer
version: 4.22
family: installing
...
In OpenShift Container Platform , you can use the Agent-based Installer to install a cluster on {oci-distributed}, so that you can run cluster workloads on infrastructure that supports dedicated, hybrid, public, and multiple cloud environments.
...
You can install an OpenShift Container Platform cluster on {oci-distributed} by using the Agent-based Installer. Red{nbsp}Hat and Oracle test, validate, and support running {oci-distributed-no-rt} workloads in an OpenShift Container Platform cluster.
...
By running your OpenShift Container Platform cluster on {oci-distributed-no-rt}, you can access the following capabilities:
...
* Instance Sizing Recommendations for OpenShift Container Platform Nodes (Oracle documentation)

[id="abi-oci-process-checklist_{context}"]
== Installation process workflow

The following workflow describes a high-level outline for the process of installing an OpenShift Container Platform cluster on {oci-distributed-no-rt} using the Agent-based Installer:
...
To run a cluster on {oci-distributed}, you must upload the generated agent ISO image to the default Object Storage bucket on {oci-distributed-no-rt}. Additionally, you must create a compute instance from the supplied base image, so that OpenShift Container Platform and {oci-distributed-no-rt} can communicate with each other for the purposes of running the cluster on {oci-distributed-no-rt}.

[NOTE]
====
{oci-distributed-no-rt} supports the following OpenShift Container Platform cluster topologies:
