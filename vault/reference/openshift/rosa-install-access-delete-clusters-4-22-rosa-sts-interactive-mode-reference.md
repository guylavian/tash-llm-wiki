---
title: "Interactive cluster creation mode reference"
type: reference
domain: openshift
slug: rosa-install-access-delete-clusters-4-22-rosa-sts-interactive-mode-reference
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/rosa_install_access_delete_clusters/rosa-sts-interactive-mode-reference
version: 4.22
...
[role="_abstract"]
This section provides an overview of the options that are presented when you use the interactive mode to create the OCM role, the user role, and OpenShift Container Platform clusters by using the {rosa-cli-first}.
...
[role="_abstract"]
You can create a OpenShift Container Platform cluster with the AWS Security Token Service (STS) by using the interactive mode. You can enable the mode by specifying the `--interactive` option when you run the `rosa create cluster` command.
...
|`OpenShift version`
|Select the version of OpenShift to install, for example . The default is the latest version. The listed `OpenShift version` options include the major, minor, and patch versions, for example `4.22.0`.
...
|`Tags (optional)`
|Specify a tag that is used on all resources created by OpenShift Container Platform in AWS. Tags can help you manage, identify, organize, search for, and filter resources within AWS. Tags are comma separated, for example: `key value, data input`.
[IMPORTANT]
====
OpenShift Container Platform only supports custom tags to Red{nbsp}Hat OpenShift resources during cluster creation. Once added, the tags cannot be removed or edited.
...
OpenShift Container Platform does not support adding additional tags outside of ROSA cluster-managed resources. These tags can be lost when AWS resources are managed by the ROSA cluster. In these cases, you might need custom solutions or tools to reconcile the tags and keep them intact.
====
...
[WARNING]
====
You cannot install a OpenShift Container Platform cluster into an existing VPC that was created by the OpenShift installer. These VPCs are created during the cluster deployment process and must only be associated with a single cluster to ensure that cluster provisioning and deletion operations work correctly.
