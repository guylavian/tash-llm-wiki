---
title: "Creating {egress-zero-title}"
type: reference
domain: openshift
slug: rosa-hcp-4-22-rosa-hcp-egress-zero-install
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/rosa_hcp/rosa-hcp-egress-zero-install
version: 4.22
family: rosa_hcp
...
[role="_abstract"]
You can create a OpenShift Container Platform cluster with {egress-zero} to pull Red{nbsp}Hat container images from a regional Amazon Elastic Container Registry (ECR) instead of from the public internet. This enhances cluster stability and security.
...
[IMPORTANT]
====
* You can use {egress-zero} on all supported OpenShift Container Platform versions. Red{nbsp}Hat recommends using the latest available z-stream release for each {ocp} version.
...
[role="_abstract"]
The OpenShift Container Platform documentation uses several terms to describe network environments with limited or no internet connectivity. These environments differ from a standard network and might require additional configuration.
...
[role="_abstract"]
You can set environment variables to streamline resource creation for your OpenShift Container Platform cluster.
...
----
+
.Suggested variables for disconnected OpenShift Container Platform clusters
...
[role="_abstract"]
You must have a Virtual Private Cloud (VPC) to create a OpenShift Container Platform cluster. To pull images from the local ECR mirror over your VPC endpoint, you must configure a privatelink service connection and modify the default security groups with specific tags.
...
[role="_abstract"]
Before you can use your VPC to create a OpenShift Container Platform cluster, you must tag your VPC subnets. Automated service preflight checks verify that these resources are tagged correctly.
...
[role="_abstract"]
You can create a OpenShift Container Platform cluster with {egress-zero} by using the {rosa-cli-first}.
...
====
`<root-acct-id>`:: If your billing account is different from your user account, add this argument and specify the AWS account that is responsible for all billing.
--

* If you set the environment variables, create a cluster with {egress-zero}, a private API, and a private Ingress by running the following command:
+
[source,terminal]
----
...
====
+
. Track the cluster creation progress by watching the OpenShift Container Platform installation program logs. To check the logs, run the following command:
+
[source,terminal]
----
...
* Egress zero Terraform VPC Example
* Terraform VPC example
* AWS `create-vpc` documentation
...
* Red{nbsp}Hat Ecosystem Catalog
* ROSA CLI firewall rules
* {hybrid-console} firewall rules
