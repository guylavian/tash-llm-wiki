---
title: "Tutorial: Configuring Microsoft Entra ID (formerly Azure Active Directory) as an identity provider"
type: reference
domain: openshift
slug: cloud-experts-tutorials-4-22-cloud-experts-entra-id-idp
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/cloud_experts_tutorials/cloud-experts-entra-id-idp
version: 4.22
...
[role="_abstract"]
You can configure Microsoft Entra ID (formerly Azure Active Directory) as the cluster identity provider in OpenShift Container Platform.
...
. Configure the OpenShift Container Platform cluster to use Entra ID as the identity provider.
...
[role="_abstract"]
To ensure that OpenShift Container Platform has enough information to create the user's account, you must configure Entra ID to give two optional claims: `email` and `preferred_username`. For more information about optional claims in Entra ID, see the Microsoft documentation.

In addition to individual user authentication, OpenShift Container Platform provides group claim functionality. This functionality allows an OpenID Connect (OIDC) identity provider, such as Entra ID, to offer a user's group membership for use within OpenShift Container Platform.
// Module included in the following assemblies:
//
// * cloud_experts_tutorials/cloud-experts-entra-id-idp.adoc
...
[NOTE]
====
In this example, the group claim includes all of the security groups that a user is a member of. In a real production environment, ensure that the groups that the group claim only includes groups that apply to OpenShift Container Platform.
====
+
...
= Configuring the OpenShift Container Platform cluster to use Entra ID as the identity provider

[role="_abstract"]
You must configure OpenShift Container Platform to use Entra ID as its identity provider. Although OpenShift Container Platform offers the ability to configure identity providers by using {cluster-manager}, use the ROSA CLI to configure the cluster's OAuth provider to use Entra ID as its identity provider. Before configuring the identity provider, set the necessary variables for the identity provider configuration.
...
[role="_abstract"]
When your first log in, you might notice that you have very limited permissions. By default, OpenShift Container Platform only grants you the ability to create new projects, or namespaces, in the cluster. Other projects are restricted from view.
...
[role="_abstract"]
OpenShift Container Platform includes a significant number of preconfigured roles, including the `cluster-admin` role that grants full access and control over the cluster.
