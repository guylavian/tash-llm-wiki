...
type: reference
domain: openshift
slug: nodes-4-22-nodes-containers-volumes
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/nodes/nodes-containers-volumes
version: 4.22
family: nodes
...
`spec.template.spec.volumes.name.v1`:: Specifies the persistent volume claim named `pvc1`.
`spec.template.spec.containers.volumeMounts`:: Specifies the container mount path.
=====
====
...
`spec.containers.volumeMounts.subPath.html`:: Specifies that HTML content is stored in the `html` folder.
