---
title: "Tutorial: {product-title} private offer acceptance and sharing"
type: reference
domain: openshift
slug: cloud-experts-tutorials-4-22-cloud-experts-rosa-with-hcp-private-offer-acceptance-and-sharing
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/cloud_experts_tutorials/cloud-experts-rosa-with-hcp-private-offer-acceptance-and-sharing
version: 4.22
...
= Tutorial: OpenShift Container Platform private offer acceptance and sharing

[role="_abstract"]
This guide describes how to accept a private offer for OpenShift Container Platform and how to ensure that all team members can use the private offer for the clusters they provision.

OpenShift Container Platform costs are composed of the AWS infrastructure costs and the OpenShift Container Platform service costs. AWS infrastructure costs, such as the EC2 instances that are running the needed workloads, are charged to the AWS account where the infrastructure is deployed. OpenShift Container Platform service costs are charged to the AWS account specified as the "AWS billing account" when deploying a cluster.

The cost components can be billed to different AWS accounts. Detailed description of how the OpenShift Container Platform service cost and AWS infrastructure costs are calculated can be found on the OpenShift Container Platform Pricing page.
...
.Procedure
. When you get a private offer for OpenShift Container Platform, you are provided with a unique URL that is accessible only by a specific AWS account ID that was specified by the seller.
+
[NOTE]
====
...
====
+
.. You can see the offer selection drop down menu with a regular private offer pre-selected in Figure 1. This type of offer can be accepted only if the OpenShift Container Platform was not activated before using the public offer or another private offer.
+
[caption="Regular private offer"]
image::rosa-regular-private-offer.png[]
+
.. You can see a private offer that was created for an AWS account that previously activated OpenShift Container Platform using the public offer, showing the product name and the selected private offer labeled as "Upgrade", that replaces the currently running contract for OpenShift Container Platform in Figure 2.
+
...
* If you do not have another active OpenShift Container Platform subscription at the time of accepting the private offer, a public offer or an older private offer entitlement, accept the private offer itself and continue with the account linking and cluster deployment steps after the specified service start date.

You must have an active OpenShift Container Platform entitlement to complete these steps. Service start dates are always reported in the UTC time zone
====

. Create or upgrade your contract.
+
.. For private offers accepted by an AWS account that does not have OpenShift Container Platform activated yet and is creating the first contract for this service, click the *Create contract button*.
+
...
+
OpenShift Container Platform entitlement is shared with other team members through your Red{nbsp}Hat organization account. All existing users in the same Red{nbsp}Hat organization are able to select the billing AWS account that accepted the private offer by following the above described steps. You can manage users in your Red{nbsp}Hat organization, when logged in as the Red{nbsp}Hat organization administrator, and invite or create new users.
+
[NOTE]
====
OpenShift Container Platform private offer cannot be shared with AWS linked accounts through the AWS License Manager.
====

. Add any users that you want to deploy OpenShift Container Platform clusters. Check this user management FAQ for more details about Red{nbsp}Hat account user management tasks.

. Verify that the already logged in Red{nbsp}Hat account includes all users that are meant to be OpenShift Container Platform cluster deployers benefiting from the accepted private offer.
...
[role="_abstract"]
When deploying OpenShift Container Platform clusters, verify that end users select the AWS billing account that accepted the private offer.

.Procedure

* When using the web interface for deploying OpenShift Container Platform, the Associated AWS infrastructure account" is typically set to the AWS account ID used by the administrator of the cluster that is being created.
...
+
[caption="Infrastructure and billing AWS account selection during OpenShift Container Platform cluster deployment"]
...
* If you try accessing a private offer that was created for the first time OpenShift Container Platform activation, while you already have OpenShift Container Platform activated using another public or private offer, and see the following notice, then contact the seller who provided you with the offer.
+
...
* John is developer who briefly tested OpenShift Container Platform by activating the service using their AWS account 123412341234, then deleted the cluster after the trial was completed.
...
* John accepts a private offer for OpenShift Container Platform as described in "Accepting a private offer".
...
* Anne wants to deploy a new OpenShift Container Platform cluster. Normally, they use their AWS account 123412341234 which is linked to the master AWS account 111111111111 for all cloud infrastructure needs.
