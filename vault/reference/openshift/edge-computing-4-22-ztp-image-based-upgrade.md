...
type: reference
domain: openshift
slug: edge-computing-4-22-ztp-image-based-upgrade
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/edge_computing/ztp-image-based-upgrade
version: 4.22
...
`FinalizeUpgrade`:: Complete the upgrade on selected clusters that completed the `Upgrade` action by moving to the `Idle` stage.
`Rollback`:: Start a rollback only on successfully upgraded clusters by moving to the `Rollback` stage.
`FinalizeRollback`:: Complete the rollback by moving to the `Idle` stage.
...
The {cgu-operator} removes the cluster labels after successfully canceling or finalizing the upgrade.
