...
type: reference
domain: sccm
slug: comanage-tutorial-co-manage-clients
tier: reference
source: https://learn.microsoft.com/en-us/intune/configmgr/comanage/tutorial-co-manage-clients
family: comanage
documentKind: "tutorial"
abstract: "Configure co-management with Microsoft Intune when you already manage Windows devices with Configuration Manager."
---

# Tutorial&#58; Enable co-management for existing clients
...
In this tutorial, you set up co-management of your Windows devices that are already enrolled in Configuration Manager. This tutorial begins with the premise that you already use Configuration Manager to manage your Windows devices.
...
- You have an on-premises Active Directory that you can connect to Microsoft Entra ID in a hybrid Microsoft Entra configuration.
  ...
  If you can't deploy a hybrid Microsoft Entra ID that joins your on-premises AD with Microsoft Entra ID, we recommend following our companion tutorial, [Enable co-management for new internet-based Windows devices](tutorial-co-manage-new-devices.md).
...
7. On the **Device operating systems** page, select the operating systems used by devices in your Active Directory environment, and then select **Next**.
   ...
   You can select the option to support Windows downlevel domain-joined devices, but keep in mind that co-management of devices is only supported for Windows devices.
8. On the **SCP** page, for each on-premises forest you want Microsoft Entra Connect to configure the service connection point (SCP), do the following steps, and then select **Next**:
   1. Select the forest.
   2. Select the authentication service.  If you have a federated domain, select AD FS server unless your organization has exclusively Windows clients and you have configured computer/device sync or your organization is using [SeamlessSSO](/azure/active-directory/hybrid/how-to-connect-sso).
...
Automatic enrollment also lets users enroll their Windows devices to Intune. Devices enroll when a user adds their work account to their personally owned device, or when a corporate-owned device is joined to Microsoft Entra ID.
...
> [!NOTE]
> Since devices are enrolled in the Microsoft Intune service based on its Microsoft Entra device token, and not a user token, only the default [Intune enrollment restriction](../../device-enrollment/restrictions.md) will apply to the enrollment.
