...
type: reference
domain: sccm
slug: osd-reduce-task-sequence-policy-size
tier: reference
source: https://learn.microsoft.com/en-us/intune/configmgr/osd/deploy-use/reduce-task-sequence-policy-size
family: osd
documentKind: "how-to"
abstract: "When the size of the task sequence policy exceeds 32 MB, the client fails to process the large policy. The client then fails to run the task sequence deployment. Manage the size of task sequence policy to reduce deployment failures."
---

# Reduce task sequence policy size
