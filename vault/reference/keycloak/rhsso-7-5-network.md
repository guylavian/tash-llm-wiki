---
title: "Chapter 8. Setting up the network - Red Hat Single Sign-On 7.5 Server Installation and Configuration Guide"
type: reference
domain: keycloak
slug: rhsso-7-5-network
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.5/html/server_installation_and_configuration_guide/network
...
# Chapter 8. Setting up the network - Red Hat Single Sign-On 7.5 Server Installation and Configuration Guide
...
If using domain mode, the commands should be executed in every host using the
/host=<host_name>/
...
In the standalone or host configuration file, the
security-realms
element should look like this:<security-realm name="UndertowRealm"> <server-identities> <ssl> <keystore path="keycloak.jks" relative-to="jboss.server.config.dir" keystore-password="secret" /> </ssl> </server-identities> </security-realm>
-
...
If using domain mode, prefix the command with the profile that is being used with:
/profile=<profile_name>/
.The resulting element,
server name="default-server"
, which is a child element ofsubsystem xmlns="urn:jboss:domain:undertow:12.0"
, should contain the following stanza:<subsystem xmlns="urn:jboss:domain:undertow:12.0"> <buffer-cache name="default"/> <server name="default-server"> <https-listener name="https" socket-binding="https" security-realm="UndertowRealm"/> ... </subsystem>
