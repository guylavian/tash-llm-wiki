---
title: "Chapter 4. Tutorials - Red Hat Single Sign-On 7.6 Red Hat Single Sign-On for OpenShift"
type: reference
domain: keycloak
slug: rhsso-7-6-tutorials
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.6/html/red_hat_single_sign-on_for_openshift/tutorials
version: 7.6
family: rhsso
...
# Chapter 4. Tutorials - Red Hat Single Sign-On 7.6 Red Hat Single Sign-On for OpenShift
...
- Rolling updates from a previous versions of Red Hat Single Sign-On for OpenShift to version 7.6.12 are not supported as databases and caches are not backward compatible.
- PostgreSQL server version 13 is required for the templates used for Red Hat Single Sign-On for OpenShift 7.6.12. If you have an obsolete PostgreSQL version, you update the PostgreSQL version before you update the database.
...
- Allow Red Hat Single Sign-On 7.6.12 to automatically migrate the database schema
- Update the database manually
By default the database is automatically migrated when you start Red Hat Single Sign-On 7.6.12 for the first time.
4.1.1. PostgreSQL version upgrade and database migration
The PostgreSQL server present in the Red Hat Single Sign-On 7.6.12 templates may be different from the previously used version. For example, consider this scenario:
...
- The Red Hat Single Sign-On 7.6.12 container images require the PostgreSQL pods to use version 13 of the PostgreSQL server.
The following procedure describes how to upgrade the PostgreSQL version to 13 and then migrate the database.
Procedure
...
Wait again for the
sso-posgresql
pod to be running and stable. Run the following commands to update the core set of Red Hat Single Sign-On 7.6.12 resources for OpenShift in the
openshift
project:If you are using an OpenShift 3.x cluster, use these commands:
...
If you are using an OpenShift 4.x cluster, use these commands:
...
Run the following command to install the Red Hat Single Sign-On 7.6.12 OpenShift image streams in the
openshift
project:$ oc -n openshift import-image rh-sso-7/sso76-openshift-rhel8:7.6 --from=registry.redhat.io/rh-sso-7/sso76-openshift-rhel8:7.6 --confirm
Update the image change trigger in the existing deployment config to reference the Red Hat Single Sign-On 7.6.12 image.
...
Start rollout of the new Red Hat Single Sign-On 7.6.12 images based on the latest image defined in the image change triggers.
...
Deploy Red Hat Single Sign-On 7.6.12 containers using the modified deployment config.
...
- Configure Red Hat Single Sign-On 7.6.12 with the correct datasource,
...
to the database job migration template namedjob-to-migrate-db-to-sso76.yaml
.NoteAlthough the
DB_SERVICE_PREFIX_MAPPING
...
) with reference to the builtsso76-db-migration-image
...
file for changes to be performed within manual database update to Red Hat Single Sign-On 7.6.12 version. Apply the database update manually.
...
Start rollout of the new Red Hat Single Sign-On 7.6.12 images based on the latest image defined in the image change triggers.
$ oc rollout latest dc/sso deploymentconfig "sso" rolled out
Deploy the Red Hat Single Sign-On 7.6.12 containers using the modified deployment config.
$ oc scale --replicas=1 dc/sso deploymentconfig "sso" scaled
...
Select sso76-ocp4-postgresql Red Hat Single Sign-On application template. When deploying the template ensure to keep the SSO_REALM variable unset (default value).
...
When performing Red Hat Single Sign-On 7.6.12 server database export, only realms and users currently in the database are exported. If the exported JSON file should include also additional Red Hat Single Sign-On realms and users, these need to be created. Use these procedures.
...
Instruct the Red Hat Single Sign-On 7.6.12 server deployed on Red Hat Single Sign-On for OpenShift image to perform database export at Red Hat Single Sign-On server boot time.
...
When the Red Hat Single Sign-On server is running as a Red Hat Single Sign-On 7.6.12 container on OpenShift, use the Admin Console Export/Import function to import the resources from a previously exported JSON file into the Red Hat Single Sign-On server’s database.
-
Log into the
master
