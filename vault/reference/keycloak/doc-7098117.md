...
abstract: "- During user authentication username & password are sent plain text from browser to back-end.- Is it possible to encrypt password before transmission to browser?- While the transfer of data is encrypted using HTTPS, there is a concerns regarding the security of sensitive information (password) being exposed.- Password is not encrypted when passing to the Keycloak server. Does it possible to encry…"
---
