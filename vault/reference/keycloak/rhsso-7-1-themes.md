---
title: "Chapter 3. Themes - Red Hat Single Sign-On 7.1 Server Developer Guide"
type: reference
domain: keycloak
slug: rhsso-7-1-themes
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.1/html/server_developer_guide/themes
...
# Chapter 3. Themes - Red Hat Single Sign-On 7.1 Server Developer Guide
...
. To change the theme used for a realm open the Admin Console
, select your realm from the drop-down box in the top left corner. Under Realm Settings
click Themes
.
To set the theme for the master
admin console you need to set the admin console theme for the master
realm. To see the changes to the admin console refresh the page.
To change the welcome theme you need to edit standalone.xml
, standalone-ha.xml
, or domain.xml
...
directory. To simplify upgrading you should not edit the bundled themes directly. Instead create your own theme that extends one of the bundle themes.
...
directory without restarting Red Hat Single Sign-On. To do this edit standalone.xml
. For theme
set staticMaxAge
to -1
and both cacheTemplates
and cacheThemes
to false
:
<theme>
...
directory. The name of the directory becomes the name of the theme. For example to create a theme called mytheme
create the directory themes/mytheme
.
...
which allows setting some configuration for the theme. For example to configure the theme themes/mytheme/login
that we just created to extend the base theme and import some common resources create the file themes/mytheme/login/theme.properties
...
3.4.5. Messages
Text in the templates are loaded from message bundles. A theme that extends another theme will inherit all messages from the parents message bundle and you can override individual messages by adding <THEME TYPE>/messages/messages_en.properties
to your theme.
...
To add a new language create the file <THEME TYPE>/messages/messages_<LOCALE>
in the directory of your theme. Then add it to the locales
property in <THEME TYPE>/theme.properties
. For a language to be available to users the realms login
, account
and email
theme has to support the language, so you need to add your language for those theme types.
...
and add:
locales=en,no
You also need to do the same for the account
and email
theme types. To do this create themes/mytheme/account/messages/messages_no.properties
and themes/mytheme/email/messages/messages_no.properties
. Leaving these files empty will result in the English messages being used. Then copy themes/mytheme/login/theme.properties
to themes/mytheme/account/theme.properties
and themes/mytheme/email/theme.properties
.
Finally you need to add a translation for the language selector. This is done by adding a message to the English translation. To do this add the following to themes/mytheme/account/messages/messages_en.properties
...
Red Hat Single Sign-On uses Freemarker Templates in order to generate HTML. You can override individual templates in your own theme by creating <THEME TYPE>/<TEMPLATE>.ftl
...
For example to create a custom login form for the mytheme
theme copy themes/base/login/login.ftl
to themes/mytheme/login
and open it in an editor. After the first line (<#import …>) add <h1>HELLO WORLD!</h1>
like so:
...
type of your theme. There’s three messages for each email. One for the subject, one for the plain text body and one for the html body.
...
or it can be deployed as an archive. During development copying the theme to the themes
...
To deploy a theme as an archive you need to create a ZIP archive with the theme resources. You also need to add a file META-INF/keycloak-themes.json
...
The deploy the archive to Red Hat Single Sign-On you can either manually create a module in modules
or use the jboss-cli
command. It’s simplest to use jboss-cli
as it creates the required directories and module descriptor for you.
To deploy mytheme.zip
on Linux run:
...
This command creates modules/org/example/mytheme/main
directory with the mytheme.zip
archive and module.xml
.
To manually create the module create the directory modules/org/keycloak/example/mytheme/main
, copy mytheme.zip
to this directory and create the file modules/org/keycloak/example/mytheme/main/module.xml
with the contents:
<?xml version="1.0" ?>
...
You also need to register the module with Red Hat Single Sign-On. This is done by editing standalone.xml
, standalone-ha.xml
, or domain.xml
. For more information on where the standalone.xml
, standalone-ha.xml
...
file resides see the Server Installation and Configuration Guide.
Then find the theme
tag under keycloak-server
subsystem and add the module to theme/modules/module
. For example:
<theme>
...
<modules>
...
If the server is running you need to restart the server after changing standalone.xml
, standalone-ha.xml
, or domain.xml
.
If the same theme is deployed to both the themes
directory and as a module the version in the themes
directory is used.
