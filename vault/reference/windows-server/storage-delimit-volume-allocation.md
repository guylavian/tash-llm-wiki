...
slug: storage-delimit-volume-allocation
tier: reference
source: https://learn.microsoft.com/en-us/windows-server/storage/storage-spaces/delimit-volume-allocation
family: storage
documentKind: "how-to"
abstract: "Learn more about: Delimit the allocation of volumes in Storage Spaces Direct"
---
...
- Your cluster uses only [three-way mirror](/azure/azure-local/concepts/fault-tolerance?context=/windows-server/context/windows-server-storage#mirroring) resiliency
...
- Your cluster uses [parity](/azure/azure-local/concepts/fault-tolerance?context=/windows-server/context/windows-server-storage#parity) or [mirror-accelerated parity](/azure/azure-local/concepts/fault-tolerance?context=/windows-server/context/windows-server-storage#mirror-accelerated-parity) resiliency
...
2. With delimited allocation, reserve the equivalent of **one capacity drive per server (with no maximum)**. This is more than the [published recommendation](/azure/azure-local/concepts/plan-volumes?context=/windows-server/context/windows-server-storage#choosing-the-size-of-volumes) for regular allocation, which maxes out at four capacity drives total.
