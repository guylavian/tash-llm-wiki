...
slug: remote-step-1-plan-the-remote-access-infrastructure
tier: reference
source: https://learn.microsoft.com/en-us/windows-server/remote/remote-access/ras/manage-remote-clients/plan/Step-1-Plan-the-Remote-Access-Infrastructure
family: remote
documentKind: "how-to"
abstract: "Learn about the steps for planning an infrastructure that you can use to set up a single Remote Access server for remote management of DirectAccess clients."
---
...
3.  Configure required adapters and addressing according to the following table. For deployments that are behind a NAT device using a single network adapter, configure your IP addresses by using only the **Internal network adapter** column.
    ...
    |Description|External network adapter|Internal network adapter<sup>1, above</sup>|Routing requirements|
    |-|--------------|------------------------|------------|
