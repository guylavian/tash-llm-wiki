---
title: "Plan outgoing email for a SharePoint Server farm - SharePoint Server"
type: reference
domain: sharepoint
slug: administration-outgoing-email-planning
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/administration/outgoing-email-planning
family: administration
documentKind: "how-to"
abstract: "How to configure outgoing email to relay email alerts and notifications in SharePoint Server."
---

# Plan outgoing email for a SharePoint Server farm - SharePoint Server
...
For more information about installing, configuring, and managing the SMTP service, see Outgoing email configuration .
...
SharePoint uses the following Service Principal Name (SPN) to authenticate to the SMTP server during Kerberos and NTLM authentication, where <host> is the SMTP server name you provided: 
 SMTPSVC/<host>
...
- The SMTP server must be configured to support STARTTLS for TLS connection encryption.

- The SMTP server must be configured to either accept or require client certificates when establishing TLS connections using STARTTLS.

- SharePoint must be configured to use TLS connection encryption to the SMTP server.

- SharePoint must be configured to use a client certificate when connecting to the SMTP server.

- SharePoint process accounts that send emails (which may include the SharePoint farm service account and the web application service account) must have permission to read the private key of the X.509 certificate.
...
Grant permission for the SharePoint authenticated email account to impersonate users

Disable SharePoint email impersonation

Use an externally secured receive connector
 ...
 **NOTE:** This permission is only necessary if SharePoint will impersonate users whose email account is not managed by your organization. |
| ms-Exch-SMTP-Accept-Authoritative-Domain-Sender | This permission allows senders that have email addresses in authoritative domains to establish a session to this receive connector. |
...
Launch the **SharePoint 2019 Management Shell**.
...
- Create a dedicated "Custom" receive connector for the SharePoint farm.

- Set the receive connector's permission group to "Exchange Servers".

- Set the receive connector's authentication type to "externally secured".
