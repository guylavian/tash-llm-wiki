---
title: "Install and configure workflow for SharePoint Server - SharePoint Server"
type: reference
domain: sharepoint
slug: governance-install-and-configure-workflow-for-sharepoint-server
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/governance/install-and-configure-workflow-for-sharepoint-server
family: governance
documentKind: "install-set-up-deploy"
abstract: "Learn how to install and configure workflow in SharePoint Server."
---

# Install and configure workflow for SharePoint Server - SharePoint Server
...
| --- | --- | --- |
| **SharePoint 2010 Workflow** | Windows Workflow Foundation 3 | Installs automatically with SharePoint Server. |
| **SharePoint 2013 Workflow** | Windows Workflow Foundation 4 | Requires SharePoint Workflow Manager or Microsoft Workflow Manager, and SharePoint Server. |
| **Project Server Workflow 2013** | Windows Workflow Foundation 4 | Requires Project server. |
| **Project Server Workflow 2016 or higher** | Windows Workflow Foundation 4 | Installs automatically with SharePoint Server. |
...
- Project Server 2016 or higher has been integrated into SharePoint Server; Project Server needs to be activated first when it's installed automatically upon the installation of SharePoint Server.

- Project server 2013 also isn't installed automatically when you install SharePoint Server. You must install and download Project Server 2013 separately. For information on how to install Project server, see Getting started developing Project Server workflows.
...
SharePoint Workflow Manager is supported in farms having an odd number of hosts, for example, 1, 3, or 5.  A farm with 2 or 4 SharePoint Workflow Manager hosts isn't supported.
...
SharePoint Workflow Manager may not be installed and configured correctly with only RODCs (read-only domain controllers) available in the network environment.  It requires an RWDC (read/write domain controller).
...
SharePoint Workflow Manager supports the version 10.1 CU7 (10.1.2841.9590) of Azure Service Fabric and higher versions.

If **Windows Fabric** is already installed on your machine, perform the following steps to do the upgrade:

- Open Workflow Manager Configuration and exit the farm.

- Uninstall old Microsoft Azure Service Fabric from the Control Panel.

- Restart the server.

- Run the following command to install Service Fabric using Workflow PowerShell opened as an administrator:

`.\MicrosoftServiceFabric.<higher_version>.exe /accepteula`

- Restart the server.

- Verify the Microsoft Azure Service Fabric version from the Control Panel.

- Rejoin the server using Workflow Configuration Manager.
...
Though it's supported to install SharePoint Workflow Manager on servers running SharePoint Server, it's recommended that SharePoint Workflow Manager is installed on its own dedicated servers for performance and reliability reasons.
...
The configuration wizard will provide a summary of your choices before they're committed.
...
If you're creating a multi-server SharePoint Workflow Manager farm, you must run the workflow configuration wizard on the other nodes and chose the "Join an Existing Workflow Manager Farm" option.
...
The App Management service can be created using Central Administration.
...
Sign-in to each server in the SharePoint Server farm.

Install the SharePoint Workflow Manager **Client** on each server in the SharePoint farm.
...
On one SharePoint server, open the SharePoint Management Shell as an administrator by right-clicking the **SharePoint Management Shell** command and choosing **Run as administrator**.

Run the **Register-SPWorkflowService** cmdlet to connect the SharePoint farm with the SharePoint Workflow Manager farm. The cmdlet should be run only once and can be run from any of the servers in the SharePoint farm.
...
Determine whether you need to install SharePoint Workflow Manager certificates on the SharePoint servers.
...
Open the SharePoint Management Shell as an administrator. This is accomplished by right-clicking the **SharePoint Management Shell** command and choosing **Run as administrator**.
...
Microsoft Workflow Manager can't be upgraded in-place, and SharePoint Workflow Manager can't be placed on top of Microsoft Workflow Manager. In order to update Microsoft Workflow Manager (Classic WFM) to SharePoint Workflow Manager (SPWFM), you must uninstall any prior versions of Workflow Manager, Workflow Manager Client, and Service Bus.

Note

You can upgrade to SharePoint Workflow Manager from any version of Microsoft Workflow Manager.
Because you're upgrading an existing "Classic WFM" farm to SPWFM, the WFM databases will be reused, and your existing registration and workflows should remain intact.

Follow the steps below to uninstall Microsoft Workflow Manager and install SharePoint Workflow Manager:

Important

Because the upgrade steps require that you disjoin and then rejoin an existing WFM farm, you will need the WFM "Certificate Generation Key", when rejoining. If you aren't sure what that key is, and haven't documented it somewhere, you may need to Reset Certificate Generation Key before proceeding.
You won't be able to join the existing workflow farm without a valid Certificate Generation Key.

Run the Workflow Manager Configuration Wizard.

Select **Leave Workflow Manager Farm**.

Confirm the subsequent steps until the end of the wizard.

Repeat this step on every Microsoft Workflow Manager server in the workflow farm.
...
Uninstall Microsoft Workflow Manager, Workflow Manager Client, Service Bus for Windows Server, and Windows Fabric if they're installed. You can uninstall them from the Control Panel. If Windows Fabric is installed, ensure you install Azure Service Fabric after uninstalling Windows Fabric.

Important

If you're installing SharePoint Workflow Manager on a SharePoint server, you may see both "Windows Fabric" and "AppFabric 1.1 for Windows Server" installed. Be sure to only uninstall Windows Fabric. **Do not uninstall AppFabric 1.1**. It's a different service, and is required for SharePoint Distributed Cache.

If the folders "*%ProgramFiles%\Workflow Manager\1.0"*  or *"%Program Files%\Service Bus\1.0"* already exist, you must manually remove them for the next steps to succeed.

Reboot the SharePoint Workflow Manager server.

If it's not already installed, use the steps from the Prerequisites section above to install Azure Service Fabric.

Install SharePoint Workflow Manager and SharePoint Workflow Manager Client. SharePoint Workflow Manager and SharePoint Workflow Manager Client can be downloaded from here.  The system requirements can be found on that page as well.

Run the Workflow Manager Configuration Wizard and choose the "Join an Existing Workflow Manager Farm" to rejoin the previous farm.  Use the database, service account, and Certificate Generation Key information used in the previous "Classic WFM" farm.

Note

When upgrading, there's typically no need to delete the existing Workflow Service Application Proxy and reconnect using the Register-SPWorkflowService cmdlet. If you encounter the invalidity of the Certificate Generation Key for SharePoint Workflow Manager and Service Bus, you may need to reset it, see Reset Certificate Generation Key.

Rerun the Workflow Manager Configuration Wizard, select **Upgrade Workflow Manager Farm**, and confirm subsequent steps until the end.
...
If there's more than one server in your Workflow Manager farm, repeat the previous steps on all workflow farm servers.

Install the SharePoint Workflow Manager **Client** on each server in the SharePoint Server farm after uninstalling any previous versions.
...
- Install SharePoint Designer 2013 on a client machine and create a workflow based on the SharePoint 2013 Workflow platform. For more information, see Creating a workflow by using SharePoint Designer 2013 and the SharePoint 2013 Workflow platform.
