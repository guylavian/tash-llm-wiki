---
title: "Create an Excel Services dashboard using a Data Model (SharePoint Server 2013) - SharePoint Server"
type: reference
domain: sharepoint
slug: administration-create-an-excel-services-dashboard-using-a-data-model-sharepoint-server-2013
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/administration/create-an-excel-services-dashboard-using-a-data-model-sharepoint-server-2013
family: administration
documentKind: "how-to"
abstract: "Learn how to create, publish and use a basic Excel Services dashboard using a Data Model that is created in Excel."
---

# Create an Excel Services dashboard using a Data Model (SharePoint Server 2013) - SharePoint Server
...
Excel Services must be configured to support Data Models in the SharePoint environment that you are using. For information about how to deploy Excel Services, see Configure Excel Services in SharePoint Server 2013 and Manage Excel Services data model settings (SharePoint Server 2013).
...
For information about how to configure Secure Store, see Plan the Secure Store Service in SharePoint Server and Configure the Secure Store Service in SharePoint Server.
...
| How will the dashboard be used? That is, what kinds of information do the dashboard consumers want to see? | Sales representatives, managers, executives, and other dashboard consumers want to use the dashboard to view, explore, and analyze data. At a minimum, the dashboard consumers want to see the following kinds of information:  
  Sales amounts across different sales territories at the country/region level  
  Order quantities for products in the Internet and reseller sales channels  
  Order quantities and sales amounts for various promotions the company held  
...
| What items should the dashboard contain? | Our example dashboard includes the following items:  
  A Data Model that includes several tables that are stored in SQL Server.  
  A report showing product sales information across different countries/regions  
  A report showing orders and sales information for different products  
...
On the **Data** tab, choose **Get External Data** group, choose **From Other Sources**, and then choose **From SQL Server**.

The Data Connection Wizard opens.

In the **Data Connection Wizard**, in the **Server name** box, specify the name of the server where the Adventure Works sample dataset is stored.

In the **Select Database and Table** dialog, in the **Select the database that contains the data that you want** list, choose **AdventureWorksDW2012**.

Select both **Connect to a specific table** and **Enable section of multiple tables**, and then select the following tables:
...
Verify that **Import relationships between tables is selected**, and then click **Next**.

In the **Save Connection and Finish** dialog, next to **Excel Services**, click **Authentication Settings**.
...
Do not click **Finish** yet. In the **Save Data Connection File and Finish** dialog, click **Browse**.

Type the address of the Business Intelligence Center site that you are using. The address typically takes the form http://servername/sites/bicenter.

Double-click **Data Connections**, and then click **Save**.

Click **Finish**. A Data Connections dialog appears.

In the **Data Connections** dialog, take the following steps:

In the **Title** box, specify a title, such as Adventure Works Data Model.

In the **Description** box, specify a description for the Data Model.

In the **Keywords** box, specify some words and phrases, such as **Adventure Works** and **Data Model**.

Click **OK** to close the Data Connections dialog.

On the **Import Data** dialog, choose **Only Create Connection**. Make sure the **Add this data to the Data Model** is selected, and then click **OK**. The Data Model is created and saved in the Data Connections library in the Business Intelligence Center site.
...
| --- | --- | --- |
| PivotChart report | Territory Sales | Bar chart report that shows sales amounts across different countries/regions. |
...
In Excel, on the **Insert** tab, click **PivotChart**.

In the **Create PivotChart** dialog, select **Use an external data source**, and then click **Choose Connection**.

In the **Existing Connections** dialog, on the **Tables** tab, select **Tables in Workbook Data Model**, and then click **Open**.

In the **Create PivotChart** dialog, choose **Existing Worksheet**, and then click **OK**. **Chart 1** opens for editing. In the PivotChart Fields section, notice the five tables that you selected for the Data Model are listed.

Expand **DimSalesTerritory**, and then select **SalesTerritoryCountry**.

Expand **FactInternetSales**, and then select **SalesAmount**. The chart updates to show sales amounts across different countries/regions.

Notice that the chart legend says **Total**, which is not necessarily useful information. To change that, follow these steps:

In the **PivotChart Fields** section, in the **Values** field well, click **Sum of SalesAmount**, and then click **Value Field Settings**.

In the **Value Field Settings** dialog, in the **Custom Name** box, type **Internet Sales**, and then click **OK**. The chart does not seem to have changed at this point, but you will see the changes after you complete the next step.

In the **PivotChart Fields** section, expand **FactResellerSales**, and then select **SalesAmount**. The chart updates to show two sets of bars representing sales amounts across different countries/regions.

To change how the sales amount information for the reseller channel is represented in the chart legend, follow these steps:

In the **PivotChart Fields** section, in the **Values** field well, click **Sum of SalesAmount**, and then click **Value Field Settings**.

In the **Value Field Settings** dialog, in the **Custom Name** box, type **Reseller Sales**, and then click **OK**.

The chart legend clearly indicates which values apply to Internet sales and which to reseller sales.

Somewhere on the chart, such as in a blank area, right-click, and then choose **PivotChart Options**.

In the **PivotChart Name** box, type a name for the report, such as **Territory Sales**, and then click **OK**.

To move the report, click a blank area in the PivotChart report, and then drag report so that its upper-left corner aligns with the upper-left corner of cell D1 in the worksheet.
...
In Excel, click cell D21. Then, on the **Insert** tab, click **PivotTable**.

On the **Create PivotTable** dialog, choose **Use an external data source**, and then click **Choose connection**.

On the **Tables** tab, select **Tables in Workbook Data Model**, and then click **Open**.

Verify that **Existing Worksheet** is selected, and then click **OK**. **PivotTable 2** opens for editing.

In the **PivotTable Fields** section, expand **DimProduct**, and then select **ModelName**. The report updates to display a list of products.

To replace the default text that says **Row Labels**, click cell D21, and type Product Model.

In the **PivotTable Fields** section, expand **FactInternetSales**, and then select **OrderQuantity**.

In the **PivotTable Fields** section, in the **Values** field well, click **Sum of OrderQuantity**, and then click **Value Field Settings**.

In the **Value Field Settings** dialog, in the **Custom Name** box, type Internet Orders, and then click **OK**.

In the **PivotTable Fields** section, expand **FactInternetSales**, and then select **SalesAmount**.

In the **PivotTable Fields** section, in the **Values** field well, select **Sum of SalesAmount**, and then click **Value Field Settings**.

In the **Format Cells** dialog, in the **Category** pane, choose **Currency**, and then click **OK**.

Click **OK** to close the **Value Field Settings** dialog. The report now shows order quantities and sales amounts for various products sold online.

In the **PivotTable Fields** list, in the **FactResellerSales** section, select **OrderQuantity**.

In the **PivotTable Fields** section, in the **Values** field well, click **Sum of OrderQuantity**, and then click **Value Field Settings**.

In the **Value Field Settings** dialog, in the **Custom Name** field, type Reseller Orders, and then click **OK**.

In the **PivotTable Fields** section, in the **FactResellerSales** section, select **SalesAmount**.

In the **PivotTable Fields** section, in the **Values** field well, select **Sum of SalesAmount**, and then click **Value Field Settings**.

In the **Custom Name** box, type Reseller Sales, and then click **Number Format**.

In the **Format Cells** dialog, in the **Category** pane, choose **Currency**, and then click **OK**.

Click **OK** to close the **Value Field Settings** dialog.

The report now shows order quantities and sales amounts for various products sold online and through resellers.

Somewhere inside the PivotTable report, right-click, and then choose **PivotTable Options**.

In the **PivotTable Name** box, type a name for the report, such as Product Orders and Sales. Clear the **Autofit column widths on update** option, and then click **OK**.
...
In Excel, click cell A1. Then, on the **Insert** tab, click **Slicer**.

In the **Existing Connections** dialog, on the **Data Model** tab, select the Data Model we've been using in this workbook, and then click **Open**.

In the **Insert Slicers** dialog, in the **DimPromotion** section, select **EnglishPromotionName**, and then click **OK**. The slicer appears on the worksheet.

Move the slicer so that its upper-left corner aligns with the upper-left corner of cell A1.

Expand the height of the slicer so that all its items are visible.

To change the default name of the slicer, on the **Options** tab, in the **Slicer Caption** box, type **Promotion**.
...
Click to select the slicer. Then, on the **Slicer Tools Options** tab, in the **Slicer** group, click **Report Connections**.

In the **Report Connections** dialog, select the **Product Orders and Sales** report and the **Territory Sales** report, and then click **OK**. The slicer is now connected to the reports.
...
In Excel, on the **File** tab, click **Save As**, and then click **Browse**.

In the **File name** box, type a name for the workbook such as Adventure Works Sales and Orders, and then click **Save**. The workbook is saved to your computer.
...
To rename the worksheet, right-click its tab where it says **Sheet1**, and then choose **Rename**. Immediately type a new name for the worksheet, such as Orders and Sales, and then press the Enter key.
...
In Excel, on the **File** tab, click **Browser View Options**.

On the **Show** tab, use the list to select **Sheets**, select **Orders and Sales**, and then click **OK**

Save the workbook, and then close Excel.
...
Select the **Territory Sales** report and the **Product Orders and Sales** reports, and then click the **Parameters** tab.

Click **Add**, select the **Promotion** slicer, and then click **OK**.

Click **OK** to close the **Browser View Options** dialog.
...
Open a web browser. In the address line, type the website address (URL) to the Business Intelligence Center site that you are using. The address typically takes the form http://servername/sites/bicenter.

Click **Site Contents**, and then click **Documents**.

Click **+ new document** to open the **Add a document** dialog.

Click **Browse** to open the **Choose File to Upload** dialog.

Select the **Adventure Works Sales and Orders** workbook, and then click **Open**.
...
Choose the **Adventure Works Sales and Orders** dashboard. The dashboard opens in a browser window.
...
| **Question** | **Action** |
| --- | --- |
| Which two countries/regions sold more on the Internet than through resellers? | Look at the bar chart. Notice that in Australia and Germany, Internet sales amounts are greater than reseller sales amounts. |
...
| What product had the highest sales amounts for the reseller channel across all promotions? | To answer this question, follow these steps.  
