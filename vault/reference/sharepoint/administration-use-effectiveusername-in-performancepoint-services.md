---
title: "Use EffectiveUserName in PerformancePoint Services - SharePoint Server"
type: reference
domain: sharepoint
slug: administration-use-effectiveusername-in-performancepoint-services
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/administration/use-effectiveusername-in-performancepoint-services
family: administration
documentKind: "how-to"
abstract: "Learn how to use the EffectiveUserName option when it connects to an Analysis Services data from PerformancePoint Services Dashboard Designer."
---

# Use EffectiveUserName in PerformancePoint Services - SharePoint Server
...
On the SharePoint Central Administration website home page, click **Security**.

On the Security page, under **General Security**, click **Configure service accounts**.

On the Service Account page, in the **Credential Management** section, from the drop-down list, select the application pool that runs PerformancePoint Services Application.
...
Type the name of the account that runs the PerformancePoint Services application pool, and then click **OK**.
...
In the Category pane of the **Select a Data Source Template** dialog, click **Multidimensional** and then click **Analysis Services**. Click **OK**.

In the left navigation pane (workspace browser), type the name of your data source.

In the **Formatting Dimension** drop-down list, select desired dimension formatting needed for the report.

In the **Cache Lifetime** drop-down box, type the refresh rate (in minutes) for the cache. Data from this data source will update at this interval.

In the center pane, click the **Editor** tab. In the **Connection Settings** section, select the method by which to connect to the data source. To use a standard connection:

Select **Use standard connection**

Type the full path for the server to which you want to connect. This populates the options in the database list.

In the **Database** box, select a database name.

(Optional) In the **Roles** box, type the name of the role, such as an administrator or database role. To specify multiple roles, type the names of the roles and separate them with commas.

From the **Cube** drop-down list, select the specific cube that you want to use as the data source in the database.

In the **Data Source Settings** section, select **Per-user identity** as the method by which to authenticate to the data source.

Click **Test Data Source** to confirm your connection is configured correctly.
