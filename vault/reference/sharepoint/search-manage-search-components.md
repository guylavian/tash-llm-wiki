---
title: "Manage search components in SharePoint Server - SharePoint Server"
type: reference
domain: sharepoint
slug: search-manage-search-components
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/search/manage-search-components
family: search
documentKind: "how-to"
abstract: "Learn how to use Windows PowerShell to manage search components in an existing search topology that has content in the SharePoint Server search index. Use these procedures to scale out or scale down the search topology of the Search service application."
---

# Manage search components in SharePoint Server - SharePoint Server
...
Start a SharePoint Management Shell. If you already have an open SharePoint Management Shell in which you have created reusable Microsoft PowerShell object references, use the open shell instead.
...
Start a SharePoint Management Shell. If you already have an open SharePoint Management Shell in which you have created reusable SharePoint Management Shell object references, use the open shell instead.
...
The command creates a clone search topology that can be referenced with  *$clone* if you continue to use the same SharePoint Management Shell to add or remove search components and to activate the search topology.

(Optional) If you have to remove search components from the search topology, you have to retrieve the search component Id. At the Microsoft PowerShell command prompt, type the following command(s):
...
*$<host n>* is the PowerShell object reference to the running search service instance on the server that you want to add the search component to. See Start a search service instance on a server.
...
Verify that the new search component was added to the clone topology. At the Microsoft PowerShell command prompt, type the command:
...
Make sure that the current active topology is healthy and that the search component that you are about to remove is **Active**. View the status of the search topology in the Search Administration page in Central Administration or run the Windows PowerShell cmdlet  `Get-SPEnterpriseSearchStatus`.
...
When prompted, confirm that you want to remove the search component.
...
Clone the active search topology. See Clone the active search topology.

Add a new search component to the server that you eventually want the search component to be hosted on. See Add a search component.

Activate the search topology. This topology will have one superfluous search component. See Activate a search topology.

Make sure that the current active topology is healthy. View the status of the search topology in the Search Administration page in Central Administration or run the Windows PowerShell cmdlet  `Get-SPEnterpriseSearchStatus`.

Clone the search topology again. See Clone the active search topology.

Remove the superfluous search component. See Remove a search component.

Activate the search topology again. See Activate a search topology.
...
Verify that your new topology is active. At the Windows PowerShell command prompt, type the following command(s):
