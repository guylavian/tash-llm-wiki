---
title: "Introduction of a new RBS provider **Remote Share Provider**. - SharePoint Server"
type: reference
domain: sharepoint
slug: administration-remote-share-provider
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/administration/remote-share-provider
family: administration
documentKind: "article"
abstract: "Learn about a new RBS provider."
---

# Introduction of a new RBS provider **Remote Share Provider**. - SharePoint Server
...
- How to backup SharePoint with DPM

- How to backup SQL with DPM

- How to backup file data with DPM

If you don't use DPM to manage backup and restore, you can follow the two steps to back up SharePoint in sequence:

- Back up farm or content database by using Backup-SPFarm PowerShell cmdlet.

- Backup remote SMB storage by using your existing backup tool.

To restore SharePoint back, reverse the sequence to:

- Restore the SMB storage from backup storage.

- Restore content database or farm by using Restore-SPFarm PowerShell cmdlet.
...
- Enable BitLocker to strengthen the data safety.
...
Share same SMB BLOB storage between active the SharePoint farm and the failover farm.
...
Share same SMB BLOB storage between active SharePoint farm and failover farm with SMB BLOB storage failover backup.
...
- The user account used to perform the steps in the Provision a BLOB store for each content database section must be a member of the `db_owner` fixed database role on each database that you're configuring RBS for.

- The user account installing the client library in the steps in the Install the RBS client library on SQL Server and each front-end or application server section must be a member of the administrators' group on all of the computers where you're installing the library.
...
- Confirm that the user account performing these steps is a member of the `db_owner` fixed database role on each database that you're configuring for RBS.
...
See Get-Credential to get PSCredential object for the RBS provider.
