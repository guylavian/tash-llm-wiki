---
title: "Display hybrid federated search results in SharePoint - SharePoint Server"
type: reference
domain: sharepoint
slug: hybrid-display-hybrid-federated-search-results-in-sharepoint-online
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/hybrid/display-hybrid-federated-search-results-in-sharepoint-online
family: hybrid
documentKind: "how-to"
abstract: "Configure a SharePoint hybrid environment so that user searches from the SharePoint in Microsoft 365 Search Center display results from both the SharePoint in Microsoft 365 and SharePoint Server search indexes."
---

# Display hybrid federated search results in SharePoint - SharePoint Server
...
On the page where you can create a new result source, do the following:
...
On the page for managing query rules, do the following:

- In the **Select a Result Source** dropdown, select the result source that you selected in step 4a of Step 2 in this article ( Step 2: Create a query rule to turn on hybrid search results in SharePoint in Microsoft 365) (for example, **Local SharePoint Results**).

A list of query rules that apply to that result source appears.

- In the list of query rules, select the query rule that you created according to step 2 in this article (Step 2: Create a query rule to turn on hybrid search results in SharePoint in Microsoft 365) (for example, Show results from SharePoint Server 2013).

On the page for editing the query rule, in the **Actions** section, in the **Result Blocks** subsection, next to the name of the query rule that will show results from the SharePoint Server search index (for example, **Show results from SharePoint Server 2013**), select **edit**.

In the **edit result block** dialog, in the **Query** section, select **Launch Query Builder**.

In the **build your query** dialog, on the **BASICS** tab, do the following:

In the **Select a query** section, select the result source that you created according to step 1 in this article ( Step 1: Create a result source that defines how to get search results from the SharePoint Server 2013 deployment) (for example, Get results from SharePoint Server).
...
Sign in to SharePoint in Microsoft 365 as a federated user who has been activated in SharePoint in Microsoft 365, and who has permissions to view the root site collection there.

Go to the enterprise Search Center in SharePoint in Microsoft 365.
...
Select a search vertical that uses a result source that you specified in step 5c of the second procedure in this article (Step 2: Create a query rule to turn on hybrid search results in SharePoint in Microsoft 365), such as **Local SharePoint Results**. That is, select a search vertical that you specified on the **Add Query Rule** page, in the **Context** section, under **Query is performed on these sources**.

On the search results page, you should see results from the search index for SharePoint in Microsoft 365 and a result block from the SharePoint Server search index.
...
If you do not see results from both search indexes on the search results page, do the following:'
...
If you still do not see search results from both search indexes, check the SharePoint in Microsoft 365 Unified Logging Service (ULS) logs, also called the SharePoint in Microsoft 365 trace logs.
