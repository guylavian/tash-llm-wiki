---
title: "Configure incoming email for a SharePoint Server farm - SharePoint Server"
type: reference
domain: sharepoint
slug: administration-incoming-email-configuration
tier: reference
source: https://learn.microsoft.com/en-us/sharepoint/administration/incoming-email-configuration
family: administration
documentKind: "how-to"
abstract: "Learn how to install and configure the SMTP service, prepare your environment, and configure incoming email for a SharePoint Server 2016, SharePoint Server 2013, and SharePoint Foundation 2013 farm."
---

# Configure incoming email for a SharePoint Server farm - SharePoint Server
...
Open **Server Manager**, select **Manage**, and then select **Add Roles and Features**.

Select **Next** until the Select features page appears, select **SMTP Server**, select **Add Features**, and then select **Next**.

On the **Confirm Installation Selections** page, select **Install**.

On the **Installation Results** page, check that the installation finished successfully, and then select **Close**.
...
Select **Next** until the Select server roles page appears, select **Management Tools** and **IIS 6 Management compatibility**, and then select **Install**.

In **Application Server** section, select **Add Role Services**.

On the Select Role Services page, select **Management Tools** and **IIS 6 Management compatibility**, and then select **Install**.
...
Open Server Manager, select **Tools**, and then select **Internet Information Services (IIS) 6.0 Manager**.

In IIS Manager, expand the server name that contains the SMTP server that you want to configure.

Right-click the SMTP virtual server that you want to configure, select **Start**, and then right-click the server again, and select **Properties**.

On the **Access** tab, in the **Access control** area, select **Authentication**.

In the **Authentication** dialog, verify that **Anonymous access** is selected, and select **OK**.

On the **Access** tab, in the **Relay restrictions** area, select **Relay**.

To enable relaying from any server, select **All except the list below**.
...
Open Server Manager, select **Tools**, and then select **Services**.

In Services, right-click **Simple Mail Transfer Protocol (SMTP)**, and then select **Properties**.

In the **Simple Mail Transfer Protocol (SMTP) Properties** dialog, on the **General** tab, in the **Startup type** list, select **Automatic**, and select **OK**.
...
In Central Administration, select **System Settings**, and in the **E-Mail and Text Messages (SMS)** section, select **Configure incoming e-mail settings**.

To enable sites on this server to receive email, on the **Configure Incoming E-Mail Settings** page, in the **Enable Incoming E-Mail** section, select **Yes**.

Select the **Automatic** settings mode.

In the **Incoming E-Mail Server Display Address** section, in the **E-mail server display address** box, enter a display name for the email server, for example, mail.fabrikam.com.

Use the default settings for all other sections, and then select **OK**.
...
In the **Central Administration** website, select **System Settings**, in the **E-Mail and Text Messages (SMS)** section, select **Configure incoming e-mail settings**.
...
If you want to connect to Directory Management Service, in the **Directory Management Service** section, select **Yes**.
...
If you want to use a remote Directory Management Service, select **Use remote** and complete the remainder of this step. Otherwise, select **No**, and proceed to step 8.
...
In the **Incoming E-Mail Server Display Address** section, in the **E-mail server display address** box, enter a display name for the email server (for example, mail.fabrikam.com). You typically use this option together with the Directory Management Service.
...
Make sure the user account that is performing this procedure is a member of the Domain Administrators group or a delegated authority for domain administration on the domain controller that is running DNS Manager.

Open Server Manager, select **Tools**, and then select **Active Directory Users and Computers**.
...
In Active Directory Users and Computers, right-click the OU that you created, and then select **Delegate control**.

On the **Welcome** page of the Delegation of Control Wizard, select **Next**.
...
In the **Select Users, Computers, and Groups** dialog, select **OK**.

On the **Users or Groups** page of the Delegation of Control Wizard, select **Next**.
...
On the last page of the Delegation of Control Wizard, to exit the wizard, select **Finish**.
...
In the Delegation of Control Wizard, select **Next**.
...
On the **Tasks to Delegate** page of the Delegation of Control Wizard, select **Create a custom task to delegate**, and then select **Next**.

Select **This folder, existing objects in this folder, and creation of new objects in this folder**, and then select **Next**.

In the **Permissions** section, select **Create all Child Objects** and **Delete all Child Objects**, and then select **Next**.
...
In Active Directory Users and Computers, select the **View** menu, and then select **Advanced Features**.

Right-click the OU, and then select **Properties**.

In the **Properties** dialog, select the **Security** tab, and then select **Advanced**.
...
In the **Permissions** area, for **Delete Subtree**, select **Allow**.

Select **OK** three times to close the **Permissions**, **Properties** dialogs, and Active Directory Users and Computers.
...
Make sure the user account that is performing this procedure is a member of the Administrators group on the local computer.

In DNS Manager, select the forward lookup zone for the domain that contains the subdomain for SharePoint Server.

Right-click the zone, and then select **New Host (A or AAAA)**.

In the **New Host** dialog, in the **Name** text box, enter the host or subdomain name for SharePoint Server.

In the **Fully qualified domain name (FQDN)** text box, enter the FQDN for the server that is running SharePoint Server. This is typically in the format  *subdomain.domain.com*.
...
In the **IP address** text box, enter the IP address to which you want the FQDN to resolve.
...
In IIS Manager, expand the SMTP server, right-click **Domains**, and on the **Action** menu, point to **New**, and then select **Domain**.

In the **New SMTP Domain Wizard** dialog, select **Alias**, and then select **Next**.

In the **Domain Name** area, in the **Name** box, enter the address of the mail that is to be received by this domain.
...
In the message that confirms the creation of the host record, select **OK**.
...
In Exchange Admin Center, select **mail flow**, highlight **send connectors**, and to open the new send connector wizard, select the **+** (Add) icon.

On the Introduction page, do the following, and then select **Next**.
...
On the **Network settings** page, select **MX record associated with recipient domain**, and then select **Next**.
...
To return to the **Address Space** page, select **Save**, and then select **Next**.

On the **Source Server** page, select the **+** (Add) icon. The **Select a Server** page appears.

Select **add**, and then select **OK**.

On the new send connector wizard, select **Finish**.
...
On the **Security** tab, under the **Group or user names** box, select **Edit**.

In the **Permissions for Drop** dialog, select **Add**.

In the **Select Users, Computers, Service Accounts, or Groups** dialog, in the **Enter the object names to select** box, enter the name of the worker process group or application pool identity account for the web application, and then select **OK**.
...
In the **Select Users, Computers, Service Accounts, or Groups** dialog, in the **Enter the object names to select** box, enter the name of the logon account for the SharePoint Timer service, and then select **OK**.
...
In the **Run** dialog, enter **Adsiedit.msc**, and then select **OK**.

In the **ADSI Edit** window, expand **ADSI Edit**, expand **Domain [DomainName]**, expand **DC=DomainName, DC=com**, and then expand **CN=Users**.

Right-click the user name to which you want to add the missing attributes, and then select **Properties**.

In the **Properties** dialog, on the **Attribute Editor** tab, double-click **Internet Encoding**.

In the **Integer Attribute Editor** dialog, enter **1310720** in the **Value** box, and then select **OK**.

In the **Properties** dialog, on the **Attribute Editor** tab, double-click **mAPIRecipient**.

In the **Boolean Attribute Editor** dialog, select **False**, and then select **OK** two times.
