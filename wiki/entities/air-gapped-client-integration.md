...
- **Java/Maven** → internal **Nexus/Artifactory** proxy (Spring Security, Quarkus
  OIDC, the EAP 8 SAML/OIDC feature packs).
- **Node/npm** → internal **Verdaccio/Artifactory** registry (`openid-client`,
  `passport-openidconnect`, `oidc-client-ts`).
...
- **keycloak-js** → its "load from the Keycloak server" pattern is **air-gap
  ...
  stays version-aligned with the server automatically.
...
- **Operator/images** from a **disconnected registry mirror** (`oc-mirror`,
  ImageContentSourcePolicy/IDMS); manual OLM upgrade approval. See
  [[operator-olm-install]] and [[custom-keycloak-image]] — use a **pre-optimized
  custom image** so the server never tries to reach out at start.
- **DB migration** is local (auto-migrate on first start) — no external dependency.
  See [[database-auto-migration]].
...
re-verify only when you next have a connected machine.
