...
LACP (IEEE 802.3ad / 802.1AX) exchanges LACPDUs that include port speed in the TLV (link-aggregation parameters). The protocol can *advertise* speed, but enforcement is a platform decision. The LACP state machine may successfully negotiate an aggregator across two ports of different speeds if the platform hardware does not reject the mismatch. The bundle forms, but the forwarding plane cannot deliver in-order delivery — the IEEE 802.3ad standard is silent on speed-matching enforcement, which Cisco (and most vendors) implement as a member-consistency check in software instead.
