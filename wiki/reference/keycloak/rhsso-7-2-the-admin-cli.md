---
title: "Chapter 19. The Admin CLI - Red Hat Single Sign-On 7.2 Server Administration Guide"
type: reference
domain: keycloak
slug: rhsso-7-2-the-admin-cli
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.2/html/server_administration_guide/the_admin_cli
...
# Chapter 19. The Admin CLI - Red Hat Single Sign-On 7.2 Server Administration Guide
...
For example, on
Linux:
$ kcadm.sh config credentials --server http://localhost:8080/auth --realm demo --user admin --client admin $ kcadm.sh create realms -s realm=demorealm -s enabled=true -o $ C=$(kcadm.sh create clients -r demorealm -s clientId=my_client -s 'redirectUris=["http://localhost:8980/myapp/*"]' -i) $ kcadm.sh get clients/$CID/installation/providers/keycloak-oidc-keycloak-json
Windows:
...
By default, the Admin CLI automatically maintains a configuration file called [filename]kcadm.config located under the user’s home directory. In Linux-based systems, the full path name is [filename]$HOME/.keycloak/kcadm.config. On Windows, the full path name is [filename]%HOMEPATH%\.keycloak\kcadm.config. You can use the --config
...
Use the
create
command against theuser-federation/instances
endpoint. Specify
kerberos
as a value of theproviderName
