---
title: "Chapter 2. Tracking instance status with health checks - Red Hat build of Keycloak 26.6 Observability Guide"
type: reference
domain: keycloak
slug: rhbk-26-6-health
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.6/html/observability_guide/health-
...
# Chapter 2. Tracking instance status with health checks - Red Hat build of Keycloak 26.6 Observability Guide

Chapter 2. Tracking instance status with health checks
...
at your proxy.
2.1. Red Hat build of Keycloak health check endpoints
Red Hat build of Keycloak exposes 4 health endpoints:
-
/health/started
- Startup probe used for initial startup of Red Hat build of Keycloak before the liveness probe takes over. -
/health/live
- Check if Red Hat build of Keycloak is running. If this fails the application should be discarded (terminated, shutdown) and started again. -
/health/ready
- Checks if Red Hat build of Keycloak is ready to process requests or not. Only direct traffic to a newly started instance once the readiness probe succeeds. If this procedure fails, the application is temporarily unavailable to serve new requests, and traffic should be directed elsewhere. -
/health
- accumulates all health check procedures in the application.
...
2.2. Enabling the health checks
...
When the health endpoints are enabled, async server bootstrap is also enabled by default, resulting in the HTTP ports being open while bootstrapping is in progress. See Configuring Red Hat build of Keycloak for production for more details.
2.3. Using the health checks
...
the HEALTHCHECK
shown is based upon the ready endpoint. This is appropriate for informational purposes or to determine if the container should receive requests. It should not be used to determine if the container needs restarted.
...
| Keycloak Initialized | Returns the status of the server initialization. | No |
For some checks, you’ll need to also enable metrics as indicated by the Requires Metrics column. To enable metrics use the metrics-enabled
option as follows:
