---
title: "Chapter 8. Managing Clients - Red Hat Single Sign-On 7.3 Server Administration Guide"
type: reference
domain: keycloak
slug: rhsso-7-3-clients
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.3/html/server_administration_guide/clients
...
# Chapter 8. Managing Clients - Red Hat Single Sign-On 7.3 Server Administration Guide
...
. In that case, you can provide the URL where client publishes it’s public key in JWK format. This is flexible because when client changes it’s keys, Red Hat Single Sign-On will automatically download them without need to re-import anything on Red Hat Single Sign-On side.
...
parameter.
The frontend client itself is not automatically added to the access token audience. This allows for easy differentiation between the access token and the ID token, because the access token will not contain the client for which the token was issued as an audience. So in he example above, the my-app
...
The term scope
is used in Red Hat Single Sign-On on few places. Various occurences of scopes are related to each other, but may have a different context and meaning. To clarify, here we explain the various scopes
