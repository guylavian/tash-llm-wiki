---
title: "Chapter 3. Multi-cluster deployments - Red Hat build of Keycloak 26.6 High Availability Guide"
type: reference
domain: keycloak
slug: rhbk-26-6-multi-cluster-introduction
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.6/html/high_availability_guide/multi-cluster-introduction-
...
# Chapter 3. Multi-cluster deployments - Red Hat build of Keycloak 26.6 High Availability Guide
...
- OpenShift version 4.18 (or later).
Amazon Aurora PostgreSQL database
- High availability with a primary DB instance in one availability zone, and a synchronously replicated reader in the second availability zone
- Version 17.5
- AWS Global Accelerator, sending traffic to both ROSA clusters
...
3.11.1.1. Measure the activity of a running Red Hat build of Keycloak instance
...
3.13.4. Connect Aurora database with Red Hat build of Keycloak
...
17 upgrades: type: InPlaceRolling
- 1
- The cluster name
- 2
...
1 locking: "PESSIMISTIC"
2 stateTransfer: chunkSize: "16" indexing: enabled: true indexed-entities: - keycloak.RootAuthenticationSessionEntity startupMode: NONE backups: site-b:
3 backup: strategy: "SYNC"
4 timeout: "4500"
5 failurePolicy: "FAIL"
6 stateTransfer: chunkSize: "16"
...
1 locking: "PESSIMISTIC"
2 stateTransfer: chunkSize: "16" indexing: enabled: true indexed-entities: - keycloak.LoginFailureEntity startupMode: NONE backups: site-b:
3 backup: strategy: "SYNC"
4 timeout: "4500"
5 failurePolicy: "FAIL"
6 stateTransfer: chunkSize: "16"
...
3.14.4. Connect Data Grid with Red Hat build of Keycloak
...
apiVersion: k8s.keycloak.org/v2beta1 kind: Keycloak metadata: labels: app: keycloak name: keycloak namespace: keycloak spec: additionalOptions: - name: cache-remote-host
...
4 secret: name: remote-store-secret key: password - name: db-driver
- 1 1
...
|---|---|---|
|
Available only when remote host is set | List | |
|
| String | |
|
Available only when remote host is set | String | |
|
Available only when remote host is set | Integer |
|
|
...
1 poolMinSize: 30
2 poolInitialSize: 30 poolMaxSize: 30 usernameSecret: name: keycloak-db-secret key: username passwordSecret: name: keycloak-db-secret key: password image: <KEYCLOAK_IMAGE_HERE>
3 startOptimized: false
4 update: strategy: Auto features: enabled: - rolling-updates:v2 - multi-site
5 additionalOptions: - name: log-console-output value: json - name: metrics-enabled
6 value: 'true' - name: event-metrics-user-enabled value: 'true' - name: cache-remote-host value: "infinispan.keycloak.svc" - name: cache-remote-port value: "11222" - name: cache-remote-username secret: name: remote-store-secret key: username - name: cache-remote-password secret: name: remote-store-secret key: password - name: db-driver value: software.amazon.jdbc.Driver - name: db-tls-mode
7 value: verify-server http: tlsSecret: keycloak-tls-secret instances: 3 truststores: aurora: configMap: name: keycloak-aurora-rootcert
8 - 1
...
- Specify the
ConfigMap
name that contains the Amazon Aurora PostgreSQL database certificate bundle. The Operator will automatic mount the file in directory/opt/keycloak/conf/truststores/configmap-<config map name>/<file-name>
3.15.3. Verify the deployment
...
apiVersion: k8s.keycloak.org/v2beta1 kind: Keycloak metadata: name: keycloak spec: hostname: hostname: $HOSTNAME
...
3.22. Managing upgrades
How to manage multi-cluster upgrades.
3.22.1. Red Hat build of Keycloak upgrades
In order to update to a new major or minor version of Red Hat build of Keycloak, it is expected that the Red Hat build of Keycloak deployment on each site is taken offline during the upgrade procedure as described in Taking a site offline. Deploying different Red Hat build of Keycloak major/minor versions on each of the sites is not supported. Neither site should be exposed to user requests via the load balancer until both Red Hat build of Keycloak deployments have been upgraded and their Data Grid clusters synchronized.
Patch upgrades can be deployed with zero-downtime. It is technically possible for upgrades to be rolled out to both sites simultaneously, however we recommend that upgrades be rolled out to one site at a time in order to simplify monitoring and debugging. For example, all nodes in site A should be upgraded to the new Red Hat build of Keycloak version before the upgrade is rolled out to nodes in site B.
See the Avoiding downtime with rolling updates guide for more details.
3.22.2. Data Grid upgrades
Upgrading Data Grid to a new major or minor version, requires the Data Grid cluster to be shutdown and redeployed on both sites. This results in Red Hat build of Keycloak downtime, as requests cannot succeed without a Data Grid cluster. The Red Hat build of Keycloak deployments should not be exposed via the load balancer until both Data Grid clusters have been upgraded, marked as online and synchronized.
As major and minor upgrades of both Red Hat build of Keycloak and Data Grid require downtime, we recommend that these upgrades be performed during the same maintenance window in order to reduce overall service downtime.
Data Grid supports zero-downtime upgrades of patch versions for local clusters, however it does not support running cross-site clusters with different patch versions on each site. Therefore, in order to upgrade Data Grid patch versions with zero-downtime, it is necessary for the upgrade to be applied as follows:
- Take site B offline
- Shutdown site B
- Perform an in-place upgrade of Data Grid on site A and wait for it to complete
- Deploy the new Data Grid version to site B
- Bring site B online
Site B will no longer be able to serve Red Hat build of Keycloak requests during steps 1-4, however the load balancer should ensure these requests are still served by site A, hence downtime is avoided.
See the Data Grid cluster upgrade documentation for more details on how to perform Data Grid in-place upgrades.
3.22.3. Database upgrades
Database upgrade semantics vary depending on the vendor chosen. In general, it should be possible for Red Hat build of Keycloak to continue to function as expected if the database natively supports zero-downtime upgrades, for example AWS Aurora. If the database does not support zero-downtime upgrades, then it is necessary for the Red Hat build of Keycloak deployment to be shutdown on both sites before the database is upgraded. Once the database upgrade has completed, it is then possible to redeploy Red Hat build of Keycloak to both sites.
When a database upgrade cannot be performed without downtime, it is recommended to also perform Red Hat build of Keycloak and Data Grid major/minor upgrades in the same maintenance window as these also require downtime.
