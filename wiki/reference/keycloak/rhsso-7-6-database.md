---
title: "Chapter 6. Setting up the relational database - Red Hat Single Sign-On 7.6 Server Installation and Configuration Guide"
type: reference
domain: keycloak
slug: rhsso-7-6-database
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.6/html/server_installation_and_configuration_guide/database
guide: server_installation_and_configuration_guide
version: 7.6
family: rhsso
documentKind: "Documentation"
abstract: "Red Hat Single Sign-On comes with its own embedded Java-based relational database called H2. This is the default database that Red Hat Single Sign-On will use to persist data and really only exists so that you can run the authentication server by default. The H2 database is intended only for example purposes. It is not a supported database, so it is not tested for database migration. We highly rec…"
---

# Chapter 6. Setting up the relational database - Red Hat Single Sign-On 7.6 Server Installation and Configuration Guide

Chapter 6. Setting up the relational database
Red Hat Single Sign-On comes with its own embedded Java-based relational database called H2. This is the default database that Red Hat Single Sign-On will use to persist data and really only exists so that you can run the authentication server by default.
...
Procedure
Create a directory structure to hold your module definition within the …/modules/ directory of your Red Hat Single Sign-On distribution.
...
Copy your database driver JAR into this directory and create an empty module.xml file within it too.
Module Directory
Open up the module.xml file and create the following XML:
Module XML
...
-
The module name should match the directory structure of your module. So, org/postgresql maps to
org.postgresql
. -
The
resource-root path
...
In domain mode, make sure you edit the profile you are using: either
auth-server-standalone
orauth-server-clustered
Within the profile, search for the
drivers
XML block within thedatasources
...
Within the
drivers
XML block, declare an additional JDBC driver.-
Assign any
name
to this driver. -
Specify the
...
package that you created earlier for the driver JAR. Specify the driver’s Java class.
...
.You’ll first need to modify the
connection-url
. The documentation for your vendor’s JDBC implementation should specify the format for this connection URL value.Define the
driver
...
character set [1]). Note that in this case, length restriction to non-special fields does not apply because columns are created to accommodate given amount of characters, not bytes. If the database default character set does not allow storing Unicode, only the special fields allow storing Unicode values.
