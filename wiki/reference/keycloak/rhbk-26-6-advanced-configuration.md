---
title: "Chapter 4. Advanced configuration - Red Hat build of Keycloak 26.6 Operator Guide"
type: reference
domain: keycloak
slug: rhbk-26-6-advanced-configuration
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.6/html/operator_guide/advanced-configuration-
...
# Chapter 4. Advanced configuration - Red Hat build of Keycloak 26.6 Operator Guide
...
transaction:
xaEnabled: false
These values are illustrative; choose values appropriate for your environment and workload.
For a list of options, see the Keycloak CRD, and for guidance on each option and its acceptable values, see All configuration.
...
set to required
, this configuration is inherited by the management interface. If you have not otherwise configured the usage of HTTP for the health endpoints, you will likely want to also set the additionalOption https-management-client-auth
to request
or none
so that a valid client certificate is not required for probe requests.
4.1.1.2. Custom environment variables
You may find a need to set custom environment variables - such as for theme properties or kc.[sh|bat]
script variables. The spec.env
field of the Keycloak CR allows you to directly set any environment variable. Logic in the operator based upon looking for value of a particular setting does not consult spec.env
, therefore do not use spec.env
for anything that has a first-class configuration in the CR or may be specified as an additionalOption
.
...
Similar to additionalOptions
you may specify either a value or reference a Secret.
4.1.2. Secret References
Secret References are used by some dedicated options in the Keycloak CR, such as a tlsSecret
...
4.1.6.1. Job Scheduling
Realm import and update Jobs inherit the scheduling of the server Pods if it is specified in the Keycloak CR spec.scheduling
field. If your server Pod scheduling is highly restrictive this may result in the Job Pods being unschedulable. In this situation you may override the Job scheduling using the Keycloak CR spec.update.scheduling
and spec.import.scheduling
fields.
...
value: /management
If you are using a custom image, the Operator is unaware of any configuration options that might’ve been specified there. For instance, the management interface may use https
, but the Operator accesses it via http
when the TLS settings are specified in the custom image. To ensure proper TLS configuration, use the tlsSecret
and truststores
...
Use the truststores stanza of the Keycloak spec to specify Secrets or ConfigMaps containing PEM encoded files, or PKCS12 files with extension .p12
, .pfx
, or .pkcs12
, for example:
apiVersion: k8s.keycloak.org/v2beta1
...
-----BEGIN CERTIFICATE-----
...
When running on a Kubernetes or OpenShift environment, the server automatically includes well-known locations of trusted certificates if they are present: /var/run/secrets/kubernetes.io/serviceaccount/ca.crt
and /var/run/secrets/kubernetes.io/serviceaccount/service-ca.crt
.
This behavior is now implemented by the server and enabled by default. You can disable it using server options in the Keycloak CR additionalOptions
, for example:
...
value: "false"
To prevent the service account token from being mounted in the pod, set automountServiceAccountToken
to false
in the Keycloak CR spec. This also disables the Kubernetes CA truststore auto-discovery described above. This option cannot be false
if you plan to use an external Infinispan cluster, the Kubernetes service accounts identity provider, or any custom provider logic that expects to implicitly use the Kubernetes API.
...
4.1.12. Parameterizing service labels and annotations
If you need to set custom labels or annotations to keycloak service you can do that through spec.http.labels
and spec.http.annotations
...
4.1.13. Configuring the Kubernetes Service name and port
By default, the operator creates a Kubernetes Service named <cr-name>-service
(for example, example-kc-service
) whose port numbers match the container ports.
You can override the Service name and expose different port numbers on the Service while the Pods continue to listen on their original ports. This is useful when you want a cleaner URL (for example, https://keycloak.mynamespace.svc
instead of https://keycloak-service.mynamespace.svc:8443
), or when migrating from an existing deployment where the Service name must remain stable.
...
port: 443
targetPort: 8443
All three fields are optional. Omitting them preserves the existing behavior exactly. Similarly, serviceHttpPort
can be used to override the port for the HTTP endpoint.
4.1.14. ServiceMonitor
A ServiceMonitor
resource is used to define how a service’s metrics are discovered and scraped by Prometheus. The Red Hat build of Keycloak Operator automatically generates a ServiceMonitor
resource for your deployment. In order for the ServiceMonitor
resource to be created, at least version v0.72.0
of the monitoring.coreos.com/v1:ServiceMonitor
Custom Resource Definition (CRD) must be installed on your OpenShift cluster.
A ServiceMonitor is only created if the metrics-enabled
option is set.
...
value: "true"
The Operator generates a ServiceMonitor
...
- The
ServiceMonitor
is created with the same name as the Keycloak CR. - 2 5 6
- The
ServiceMonitor
is always deployed to the same namespace as the Keycloak CR and will only match a Service in that namespace. - 3
- The configured path defaults to
/metrics
, but respects thehttp-management-relative-path
value if configured. - 4
- The configured scheme is changed automatically depending on whether TLS is enabled or not.
4.1.14.1. Modify ServiceMonitor configuration
You can configure the interval
and scrapeTimeout
fields and also annotations
and labels
of the generated ServiceMonitor
by modifying the Keycloak CR spec.
...
annotation1: annotation-value1
annotation2: annotation-value2
You can use service monitor labels to determine which Prometheus instance will scrape metrics from Red Hat build of Keycloak.
4.1.14.2. Disable ServiceMonitor creation
