---
title: "Chapter 2. OpenID Connect - Red Hat Single Sign-On 7.2 Securing Applications and Services Guide"
type: reference
domain: keycloak
slug: rhsso-7-2-openid-connect-3
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_single_sign-on/7.2/html/securing_applications_and_services_guide/openid_connect_3
...
# Chapter 2. OpenID Connect - Red Hat Single Sign-On 7.2 Securing Applications and Services Guide
...
JBoss EAP should be running when you install the adapter. If you have either one running, you must stop it before installing and then restart it after installation is complete.
...
$ cd $EAP_HOME
$ unzip rh-sso-7.2.7.GA-eap7-adapter.zip
...
$ cd $EAP_HOME
$ unzip rh-sso-7.2.7.GA-eap6-adapter.zip
...
The only supported version of Fuse 6 is JBoss Fuse 6.3.0 Rollup 5. If you use earlier versions of Fuse 6, it is possible that some functions will not work correctly. In particular, the Hawtio integration will not work with earlier versions of Fuse 6.
...
Start JBoss Fuse 6.3.0 Rollup 5; then in the Karaf terminal type:
features:addurl mvn:org.keycloak/keycloak-osgi-features/3.4.17.Final-redhat-00001/xml/features features:install keycloak
...
. However, upgrading to JBoss Fuse 6.3.0 Rollup 5 is recommended.- Ensure that the features were installed:
...
system
directory. You can overwrite all existing jar files.Use this for JBoss Fuse 6.3.0 Rollup 5:
cd /path-to-fuse/jboss-fuse-6.3.0.redhat-254 unzip -q /path-to-adapter-zip/rh-sso-7.2.7.GA-fuse-adapter.zip
...
org.keycloak.adapters.jetty;version="3.4.17.Final-redhat-00001", org.keycloak.adapters;version="3.4.17.Final-redhat-00001", org.keycloak.constants;version="3.4.17.Final-redhat-00001", org.keycloak.util;version="3.4.17.Final-redhat-00001", org.keycloak.*;version="3.4.17.Final-redhat-00001", *;resolution:=optional
...
must contain at least these imports:org.keycloak.adapters.jetty;version="3.4.17.Final-redhat-00001", org.keycloak.adapters;version="3.4.17.Final-redhat-00001", org.keycloak.constants;version="3.4.17.Final-redhat-00001", org.keycloak.util;version="3.4.17.Final-redhat-00001", org.keycloak.*;version="3.4.17.Final-redhat-00001", *;resolution:=optional
...
org.keycloak.*;version="3.4.17.Final-redhat-00001",
...
org.eclipse.jetty.util.security;version="[8,10)",
org.keycloak.*;version="3.4.17.Final-redhat-00001",
org.keycloak.adapters.jetty;version="3.4.17.Final-redhat-00001",
*;resolution:=optional
...
features:addurl mvn:org.keycloak/keycloak-osgi-features/3.4.17.Final-redhat-00001/xml/features features:install keycloak-jaas
Log in using SSH as
admin
...
algorithm, by default. However, by default, it is currently used in JBoss Fuse 6.3.0 Rollup 5.
...
Start JBoss Fuse 6.3.0 Rollup 5 and install the keycloak feature if you have not already done so. The commands in Karaf terminal are similar to this example:
features:addurl mvn:org.keycloak/keycloak-osgi-features/3.4.17.Final-redhat-00001/xml/features features:install keycloak
...
JBoss Fuse 7 leverages Undertow adapter which is essentially the same as JBoss EAP 7 Adapter as JBoss Fuse 7.0.1 is bundled with Undertow HTTP engine under the covers and Undertow is used for running various kinds of web applications.
...
To install the keycloak feature using the Maven repository, complete the following steps:
Start JBoss Fuse 7.0.1; then in the Karaf terminal type:
feature:repo-add mvn:org.keycloak/keycloak-osgi-features/3.4.17.Final-redhat-00001/xml/features feature:install keycloak-pax-http-undertow keycloak-jaas
...
Unzip it into the root directory of JBoss Fuse. The dependencies are then installed under the
system
directory. You can overwrite all existing jar files.Use this for JBoss Fuse 7.0.1:
cd /path-to-fuse/jboss-fuse-7.0.z unzip -q /path-to-adapter-zip/rh-sso-7.2.7.GA-fuse-adapter.zip
Start Fuse and run these commands in the fuse/karaf terminal:
feature:repo-add mvn:org.keycloak/keycloak-osgi-features/3.4.17.Final-redhat-00001/xml/features feature:install keycloak-pax-http-undertow keycloak-jaas
-
...
On some later operating systems, you might also need to use the SSH command’s -o option -o HostKeyAlgorithms=+ssh-dss
because later SSH clients do not allow use of the ssh-dss
algorithm, by default. However, by default, it is currently used in JBoss Fuse 7.0.1.
Note that the user needs to have realm role admin
to perform all operations or another role to perform a subset of operations (for example, the viewer role that restricts the user to run only read-only Karaf commands). The available roles are configured in $FUSE_HOME/etc/org.apache.karaf.shell.cfg
or $FUSE_HOME/etc/system.properties
.
...
andauth-server-url
properties according to your Red Hat Single Sign-On environment. This file is used by the adapters on the server (JAAS Login module) side.{ "realm" : "demo", "resource" : "jaas", "bearer-only" : true, "auth-server-url" : "http://localhost:8080/auth", "ssl-required" : "external", "use-resource-role-mappings": false, "principal-attribute": "preferred_username" }
Start JBoss Fuse 7.0.1, install the Keycloak feature. Then type in the Karaf terminal:
...
none
. To always require re-authentication and ignore SSO, set this option tologin
. -
...
"dependencies": {
"keycloak-connect": "file:keycloak-connect-3.4.6.Final-redhat-1.tgz"
}
2.3.2. Usage
