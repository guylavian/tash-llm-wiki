---
title: "Chapter 13. Managing OpenID Connect and SAML Clients - Red Hat build of Keycloak 26.4 Server Administration Guide"
type: reference
domain: keycloak
slug: rhbk-26-4-assembly-managing-clients-server-administration-guide
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.4/html/server_administration_guide/assembly-managing-clients_server_administration_guide
...
# Chapter 13. Managing OpenID Connect and SAML Clients - Red Hat build of Keycloak 26.4 Server Administration Guide
...
- Admin URL
- Callback endpoint for a client. The server uses this URL to make callbacks like pushing revocation policies, performing backchannel logout, and other administrative operations. For Red Hat build of Keycloak servlet adapters, this URL can be the root URL of the servlet application. The callback messages sent to this URL are sent in the Red Hat build of Keycloak specific format, which is not OIDC standard. This format is supported only for clients secured by the legacy Red Hat build of Keycloak Java OIDC adapters or by the Elytron Wildfly OIDC adapter.
...
- (blank)
- Red Hat build of Keycloak does not apply PKCE unless the client sends the appropriate PKCE parameters to Red Hat build of Keycloak authorization endpoint. So PKCE is still possible to use, but it is not required.
- S256
...
Signed JWT issued by an Identity Provider is Technology Preview and is not fully supported. This feature is disabled by default.
To enable start the server with --features=preview
or --features=client-auth-federated
...
Note that this Red Hat build of Keycloak switch maps to the switch dpop_bound_access_tokens
...
- A logout request is sent to a non-OIDC compliant Red Hat build of Keycloak proprietary logout endpoint with a holder-of-key refresh token. This verification is done only for public clients as described above.
-
Request is sent to another Red Hat build of Keycloak endpoint secured by the bearer token sent in the
Authorization
header. This applies for example to the admin REST or account REST endpoints.
Red Hat build of Keycloak also provides the client policy executor dpop-bind-enforcer
...
13.1.6.1. Rules for client secret rotation
...
13.1.7. Creating an OIDC Client Secret Rotation Policy
...
13.1.8. Using a service account
...
header is Base64 encoded value of clientId and clientSecret in the format prescribed by the Authorization: Basic
header. In this example, the client ID is product-sa-client
and the client secret was password
and hence the value was obtained for example by this command in the Unix platform:
...
and client_secret
of the POST request. For other client credentials methods, the format of the parameters would be different as described above.
...
13.1.9. Role mappings in the token
...
13.1.9.1. Roles assigned to the user
...
13.1.9.2. Role protocol mappers
...
13.1.10.1. Ability for the service to call another service
...
13.1.10.3. Automatically add audience based on client roles
...
13.1.10.3.1. Example - token role mappings and audience claim
...
13.1.10.5. Token introspection audience validation
The OAuth2 token introspection endpoint validates that the authenticated client is present in the token’s audience (aud
) claim before allowing introspection. This prevents clients from introspecting tokens that were not intended for them.
If you need to disable this validation during migration, you can enable a backwards compatibility option in OpenID Connect Compatibility Modes by enabling Allow token introspection without audience check on the client that performs the introspection. See OpenID Connect provider configuration for the server-wide option.
...
Metadata descriptor URL
and cached in Red Hat build of Keycloak. If a specific certificate is requested to validate a signature (usually inPOST
...
The key used to encrypt the assertions is controlled in the same way as in the case of Client Signature Required. If Use metadata descriptor URL is enabled, the key is doenloaded and cached by Red Hat build of Keycloak. If that option is disabled, you need to import or generate the key when activating the Encrypt Assertions option.
13.2.3. Advanced tab
...
, which can be useful especially for lightweight access tokens. See the details of Token Introspection endpoint in the securing apps section. - UserInfo endpoint restriction
...
Exchange the lightweight access token for a full access token using Token Exchange, then call UserInfo with the full token
...
- Validation of client configurations
- Red Hat build of Keycloak supports validation whether the client follows settings like Proof Key for Code Exchange, Request Object Signing Algorithm, Holder-of-Key Token, and so on some endpoints like Authorization Endpoint, Token Endpoint, and so on. These can be specified by each setting item (on Admin Console, switch, pull-down menu and so on). To make the client application secure, the administrator needs to set many settings in the appropriate way, which makes it difficult for the administrator to secure the client application. Client Policies can do these validation of client configurations mentioned just above and they can also be used to autoconfigure some client configuration switches to meet the advanced security requirements. In the future, individual client configuration settings may be replaced by Client Policies directly performing required validations. Check the dedicated section to see a use case on how to protect the server by validating client uris.
...
- Enforce client URIs to comply with security criteria, for example to prevent SSRF attacks allowing only trusted patterns or domains. Check the dedicated section for more details.
