---
title: "Chapter 12. Configuring and using token exchange - Red Hat build of Keycloak 26.2 Securing Applications and Services Guide"
type: reference
domain: keycloak
slug: rhbk-26-2-token-exchange
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.2/html/securing_applications_and_services_guide/token-exchange-
...
# Chapter 12. Configuring and using token exchange - Red Hat build of Keycloak 26.2 Securing Applications and Services Guide

Chapter 12. Configuring and using token exchange
...
- Legacy token exchange: version 1 (V1) - This preview feature is not enabled by default once Red Hat build of Keycloak server is started.
...
The standard token exchange supports only use-case (1). The legacy token exchange support the four use-cases, but it is a preview feature. Therefore, the standard token exchange V2 is recommended since it is supported and will be maintained for the future. The legacy token exchange is useful for last three use cases, but it may not be backwards compatible with future Red Hat build of Keycloak versions. You can also enable both token exchange features and use them together. For example, you could use both internal-internal exchange provided by V2 together with other use cases that are supported by V1. For more details, see this token exchange comparison.
If you still need legacy token exchange feature, you also need Fine-grained admin permissions version 1 (FGAP:v1) enabled because version 2 (FGAP:v2) does not have support for token exchange permissions. This is on purpose because token-exchange is conceptually not really an "admin" permission and hence there is no plan to add token exchange permissions to FGAP:v2.
12.1. Standard token exchange
...
. The token has to many permissions; therefore, the
requester-client
...
12.1.1.1. Example token exchange request
...
12.1.2. How to enable token exchange
...
Figure 12.1. Enabling Token Exchange
12.1.3. Request and response parameters
...
will be also filtered due it contains some client roles, but at the same time, it does not contain any client roles of requested audience client target-client2
. So token would be like:
{
"azp": "requester-client",
...
12.1.5. Token exchange - Additional details
...
Figure 12.2. Enabling refresh token in Token Exchange
-
...
-
Revocation of
12.1.6. Comparison of standard token exchange and legacy token exchange
...
|
Allowed | Access token (default), Refresh token, ID token | Access token, Refresh token (default), SAML2 assertion |
|
Behaviour of | Aligned with other grants. Scope parameter means requesting optional scopes of the client, which sent the token exchange request | Scope parameter based on scopes of the "target" client specified by audience parameter. Downscoping support only |
|
Behavior of | Support for more values as per the specification. Can be used to narrow down the available audiences and keep only the requested audiences. Effectively downscoping the token per the required target audience | Support for single audience value. Token effectively issued to the client requested by audience parameter and using scopes of that client |
...
To use more than the Internal Token to Internal Token Exchange flow, also enable the admin-fine-grained-authz
feature. For details, see the Enabling and disabling features chapter.
12.2.1. How token exchange works
...
12.2.1.2. Responses from a token exchange request
...
12.2.2. Internal token to internal token exchange
...
12.2.2.1. Granting permission for the exchange
...
Figure 12.6. Client Policy Creation
...
Figure 12.7. Apply Client Policy
...
12.2.3. Internal token to external token exchange
...
12.2.3.1. Granting permission for the exchange
...
Figure 12.8. Identity Provider Permission
Procedure
Toggle Permissions Enabled to On.
Figure 12.9. Identity Provider Permission
The page displays token-exchange link.
Click the link to start defining the permission.
This setup page appears.
Figure 12.10. Identity Provider Exchange Permission Setup
- Click Client details in the breadcrumbs at the top of the screen.
Click Policies tab to create a client policy.
Figure 12.11. Client Policy Creation
...
Figure 12.12. Apply Client Policy
Your client now has permission to invoke. If you do not do this correctly, you will get a 403 Forbidden response if you try to make an exchange.
12.2.3.2. Making the request
...
12.2.4. External token to internal token exchange
...
12.2.4.1. Granting permission for the exchange
...
(issuer) claim within the JWT which must be the alias of the provider, or a registered issuer within the providers configuration.
...
12.2.5.1. Granting permission for the exchange
...
12.2.6.1. Granting permission for the exchange
If the audience
...
Figure 12.13. User Permissions
Toggle Permissions Enabled to On.
Figure 12.14. Identity Provider Permission
The page displays an impersonate link.
Click that link to start defining the permission.
This setup page displays.
Figure 12.15. Users Impersonation Permission Setup
- Click Client details in the breadcrumbs at the top of the screen.
- Define a policy for this permission.
Go to the Policies tab and create a client policy.
Figure 12.16. Client Policy Creation
- Enter the starting client that is the authenticated client that is requesting a token exchange.
Return to the users' impersonation permission and add the client policy you just defined.
Figure 12.17. Apply Client Policy
...
-d "requested_subject=wburke" \
http://localhost:8080/realms/myrealm/protocol/openid-connect/token
12.2.7. Expand permission model with service accounts
...
12.2.8. Exchange vulnerabilities
