...
type: reference
domain: openshift
slug: authentication-4-22-index
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/authentication/index
version: 4.22
...
To control access to an OpenShift Container Platform cluster,
...
An administrator can configure authentication by configuring an identity provider. You can define any supported identity provider in OpenShift Container Platform and add it to your cluster.
...
* Assigning a cluster role to a user or group: OpenShift Container Platform includes a set of default cluster roles. You can add them to a user or group.
...
* Creating cluster-admin and dedicated-admin users: The user who created the OpenShift Container Platform cluster can grant access to other `cluster-admin` and `dedicated-admin` users.
