...
type: reference
domain: openshift
slug: security-4-22-cert-manager-operator-issuer-acme
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/security/cert-manager-operator-issuer-acme
version: 4.22
family: security
...
* You have set up a {gcp-full} service account with a desired role for {gcp-full} DNS. For more information, see {gcp-full} DNS in the upstream cert-manager documentation.
+
[NOTE]
====
You can use {gcp-full} DNS with explicit credentials in an OpenShift Container Platform cluster that is not running on {gcp-short}.
====
