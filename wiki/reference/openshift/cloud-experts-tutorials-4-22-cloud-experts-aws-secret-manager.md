---
title: "Tutorial: Using AWS Secrets Manager CSI on {product-title} with STS"
type: reference
domain: openshift
slug: cloud-experts-tutorials-4-22-cloud-experts-aws-secret-manager
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/cloud_experts_tutorials/cloud-experts-aws-secret-manager
version: 4.22
...
= Tutorial: Using AWS Secrets Manager CSI on OpenShift Container Platform with STS

[role="_abstract"]
The AWS Secrets and Configuration Provider (ASCP) provides a way to expose AWS Secrets as Kubernetes storage volumes. With the ASCP, you can store and manage your secrets in Secrets Manager and then retrieve them through your workloads running on OpenShift Container Platform.
...
* A OpenShift Container Platform cluster deployed with STS
* Helm 3
* `aws` CLI
* `oc` CLI
* `jq` CLI
...
[role="_abstract"]
Before creating your application, you need to gain access to your OpenShift Container Platform cluster.

.Procedure
. Log in to your OpenShift Container Platform cluster by running the following command:
+
[source,terminal]
----
...
See
Red{nbsp}Hat documentation on creating an STS cluster before continuing this process.
Red{nbsp}Hat documentation on creating an STS cluster before continuing this process.
