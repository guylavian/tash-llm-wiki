...
type: reference
domain: openshift
slug: updating-4-22-understanding-update-channels-release
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/updating/understanding-update-channels-release
version: 4.22
family: updating
...
OpenShift Container Platform  offers the following update channels:
...
OpenShift Container Platform  offers the following update channel:
...
The `fast-` channel is updated with new versions of OpenShift Container Platform  as soon as Red{nbsp}Hat declares the version as a general availability (GA) release. As such, these releases are fully supported and purposed to be used in production environments.

[id="stable-version-channel_{context}"]
== stable- channel

While the `fast-` channel contains releases as soon as their errata are published, releases are added to the `stable-` channel after a delay. During this delay, data is collected from multiple sources and analyzed for indications of product regressions. Once a significant number of data points have been collected, these releases are added to the stable channel.
...
The `candidate-` channel offers unsupported early access to releases as soon as they are built. Releases present only in candidate channels
...
[role="_abstract"]
If you manage the container images for your OpenShift Container Platform clusters yourself, you must consult the Red{nbsp}Hat errata that is associated with product releases and note any comments that impact updates.
...
The service recommends only updates that have been tested and have no known serious regressions. For example, if your cluster is on .1 and OpenShift Container Platform suggests .4, then it is recommended to update from .1 to .4.

[IMPORTANT]
====
Do not rely on consecutive patch numbers. In this example, .2 is not and never was available in the channel, therefore updates to .2 are not recommended or supported.
====
...
* Your cluster is still supported if you change from the `stable-` channel to the `fast-` channel.

* You can switch to the `candidate-` channel at any time, but some releases for this channel might be unsupported.

* You can switch from the `candidate-` channel to the `fast-` channel if your current release is a general availability release.

* You can always switch from the `fast-` channel to the `stable-` channel. There is a possible delay of up to a day for the release to be promoted to `stable-` if the current release was recently promoted.
