---
title: "Installing a cluster on AWS into a specialized region"
type: reference
domain: openshift
slug: installing-4-22-installing-aws-specialized-region
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/installing/installing-aws-specialized-region
version: 4.22
family: installing
...
[WARNING]
====
In OpenShift Container Platform , the installation program uses Cluster API instead of Terraform to provision cluster infrastructure during installations on AWS. Installing a cluster on AWS into a secret or top-secret region by using the Cluster API implementation has not been tested as of the release of OpenShift Container Platform . This document will be updated when installation into a secret region has been tested.
...
If you are deploying an OpenShift Container Platform cluster using an AWS Marketplace image in a government region, you must first subscribe through {aws-short}. Subscribing to the offer provides you with the AMI ID that the installation program uses to deploy compute nodes.
...
You cannot use the OpenShift Container Platform installation program to create the installation configuration file. The installer does not list an AWS region without native support for an {op-system} AMI.
...
If you use a custom VPC, you must correctly configure it and its subnets for the installation program and the cluster to use. See Create a VPC in the {aws-full} documentation for more information about {aws-short} VPC console wizard configurations and creating and managing an {aws-short} VPC.
...
The {op-system} VMDK file must be the highest version that is less than or equal to the OpenShift Container Platform version you are installing.
...
----
<1> The {op-system} VMDK version, like `.0`.
