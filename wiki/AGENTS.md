...
- Raw ground-truth lives **in this vault**: `reference/<domain>/` (the folded-in
  ...
  op in `CLAUDE.md` (grep `reference/<domain>/`, or `python3 -m wikikb kb --domain
  <d> search "<terms>"`).
- **Operations** are packaged as Agent Skills in `../.skills/` (`wiki-ingest`,
  ...
  truth. Tooling (stdlib only) lives in `_meta/wikikb/`:
  - `python3 -m wikikb lint [--status]` — health check + delta-manifest audit
  - `python3 -m wikikb manifest {seed,status,record}` — delta manifest
