...
```
<repo-root>/  ← the Obsidian vault root IS the repo root (open it in Obsidian); rules ALL the data
    ├── CLAUDE.md           # this schema (single source of truth for the operations)
    ├── SKILL.md · AGENTS.md # skill trigger manifest + agent bootstrap pointer
    ├── references/         # curated reference guides (ref: tier — in-vault since 2026-07-07)
...
`claude mcp add wikikb -- python3 -m wikikb mcp` (cwd `_meta/`, or set `PYTHONPATH`).
