...
immutable raw tiers inside the vault (`reference/<domain>/`, `_sources/<domain>/`) and `references/` (in-vault). It follows Andrej
...
- **`reference/<domain>/`** — IMMUTABLE imported doc bodies, one note per source,
  plus `_gated-kb-index.md`. Never edit these.
- **`_sources/<domain>/`** — raw note staging for notes-first domains. Also immutable.
- **`references/`** — curated reference guides (the `ref:` tier). Immutable.
- **`_meta/`** — tooling, **NOT content**: excluded from every scanner. Holds
  `taxonomy.md` (the controlled vocab every page's `domain:` is validated against),
  `.manifest.json` (delta manifest), `eval/` (cases + committed goldens),
  `wikikb/` (the toolchain package, grouped into concern subpackages), `tests/`.
- **`index.md`** is the global router; **`index.<domain>.md`** is generated by
