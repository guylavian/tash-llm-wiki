---
title: "Chapter 11. Managing access to realm resources - Red Hat build of Keycloak 26.4 Server Administration Guide"
type: reference
domain: keycloak
slug: rhbk-26-4-admin-permissions
tier: reference
source: https://docs.redhat.com/en/documentation/red_hat_build_of_keycloak/26.4/html/server_administration_guide/admin_permissions
...
# Chapter 11. Managing access to realm resources - Red Hat build of Keycloak 26.4 Server Administration Guide
...
Admins with the create-realm
role who do not have the admin
role will also need the realm-specific query-realms
role to be able to create and list realms in the Admin Console.
...
11.3.1. Understanding the different types of realm administrators
There are three different types of realm administrators when managing a realm:
-
Server administrators: These are users (or service accounts) that have been granted with the
admin
role in the master realm. -
Realm administrators: These are users (or service accounts) that have been granted with the
realm-admin
role in a specific realm. -
Delegated realm administrators: These are users (or service accounts) that do not have the
realm-admin
role but have been granted access to manage a realm through the fine-grained admin permissions feature.
...
Be aware that both server and realm administrators are not affected by the permissions you define when managing access to realm resources through this feature. Always make sure to review the users granted with the admin
or realm-admin
roles to avoid any potential privilege escalation.
11.3.2. Understanding the Realm Resource Types
...
11.3.3. Understanding the scopes of access
...
|---|---|---|
| view | Defines if a realm administrator can view users. This scope should be set whenever you want to make users available from queries. |
|
...
|
| reset-password |
Defines if a realm administrator can reset user passwords. If no permission with | None |
...
11.3.3.3. Clients Resource Type
...
11.3.4. Enabling admin permissions to a realm
...
11.3.5.1. Defining permissions for viewing realm resources
...
or view-clients
. For instance, it will not happen for administrators granted with the admin
role at the master realm (server administrators), or realm administrators granted with the realm-admin
role in a realm other than the master realm.
...
11.3.5.2. Searching Permissions
...
These capabilities allow realm administrators to perform queries on their universe of permissions and identify which ones are enforcing access to a set of one or more realm resources and their scopes. Combined with the evaluation tool on the Evaluation tab, they provide a key management tool for managing permissions in a realm. See Evaluating Permissions for more details.
11.3.6. Managing Policies
...
11.3.7.1. Resolving conflicting permissions
...
group themselves.
11.3.8. Accessing a Realm administration console as a Realm Administrator
Realm administrators can access a dedicated realm-specific administration console that allows them to manage resources within their assigned realm. This console is separate from the main Red Hat build of Keycloak Admin Console, which is typically used by realm administrators.
...
11.3.8.1. Roles and Permission relationship
...
11.3.9. Understanding some common use cases
...
group.
11.3.9.1. Allowing to manage users by group of administrators
...
11.3.9.2. Allowing to manage users by group of admins but not group members
...
- Save the permission
11.3.9.3. Allowing to impersonate users for members of a group with a specific role assigned
...
11.3.9.4. Blacklisting specific users from being impersonated
...
11.3.9.5. Allowing to view users but not managing them for admins with a defined role assigned
...
11.3.9.6. Allowing to manage users and role assignment for members of a group
- Create a "User Permission" with the manage, map-roles scopes for all users.
-
Create a "Group Policy" allowing access to members of
test-admins
.
11.3.9.7. Allowing to view and manage members of a group but not members of its subgroups
-
...
11.3.9.8. Allowing to impersonate members of a specific group
-
...
mygroup-helpdesk
to it.
11.3.10. Performance considerations
