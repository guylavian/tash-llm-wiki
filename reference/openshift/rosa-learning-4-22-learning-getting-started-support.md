---
title: "Obtaining support"
type: reference
domain: openshift
slug: rosa-learning-4-22-learning-getting-started-support
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/rosa_learning/learning-getting-started-support
version: 4.22
family: rosa_learning
documentKind: "Documentation"
---

# Obtaining support

[id="learning-getting-started-support"]
= Obtaining support

[role="_abstract"]
Resolve unexpected issues and maintain system health by utilizing the provided support resources. Exploring these materials helps you quickly access the right assistance when you need help.

// Module included in the following assemblies:
//
// * rosa_learning/creating_cluster_workshop/learning-getting-started-support.adoc
[id="learning-getting-started-support-contacts_{context}"]
= Adding support contacts

[role="_abstract"]
You can add additional email addresses for communications about your cluster.

.Procedure
. On the {cluster-manager-first} user interface (UI), click *Cluster List* in the side navigation tabs.
. Select the cluster that needs support.
. Click the *Support* tab.
. Click *Add notification contact*, and enter the additional email addresses.
// Module included in the following assemblies:
//
// * rosa_learning/creating_cluster_workshop/learning-getting-started-support.adoc
[id="learning-getting-started-support-ui_{context}"]
= Contacting Red{nbsp}Hat for support using the UI

[role="_abstract"]
You can request support within {cluster-manager-first}.

.Procedure

. On the {cluster-manager-url} UI, click the *Support* tab.
. Click *Open support case*.
// Module included in the following assemblies:
//
// * rosa_learning/creating_cluster_workshop/learning-getting-started-support.adoc
[id="learning-getting-started-support-page_{context}"]
= Contacting Red{nbsp}Hat for support using the support page

[role="_abstract"]
You can request Red{nbsp}Hat support using the support page.

.Procedure
. Go to the Red{nbsp}Hat support page.
. Click *Open a new Case*.
+
image::obtain-support-case.png[]

. Log in to your Red{nbsp}Hat account, then select the reason for contacting support.
+
image::obtain-support-reason.png[]

. Select *Red{nbsp}Hat OpenShift Service on AWS*, then click *Continue*.
+
image::obtain-support-select-rosa.png[]

. Enter a summary of the issue and the details of your request. Upload any files, logs, and screenshots. The more details you provide, the better Red{nbsp}Hat support can help your case.
+
[NOTE]
====
Relevant suggestions that might help with your issue will appear at the bottom of this page.
====
+
image::obtain-support-summary.png[]

. Click *Continue*.
. Answer the questions in the new fields, then click *Continue*.
. Enter the following information about your case:
.. *Support level:* Premium
.. *Severity:* Review the Red{nbsp}Hat Support Severity Level Definitions to choose the correct one.
.. *Group:* If this is related to a few other cases you can select the corresponding group.
.. *Language*
.. *Send notifications:* Add any additional email addresses to keep notified of activity.
.. *Red{nbsp}Hat associates:* If you are working with anyone from Red{nbsp}Hat and want to keep them in the loop you can enter their email address here.
.. *Alternate Case ID:* If you want to attach your own ID to it you can enter it here.
. Click *Continue*.
. On the review screen make sure you select the correct cluster ID that you are contacting support about.
+
image::obtain-support-cluster-id.png[]

. Click *Submit*.

. You will be contacted based on the response time committed to for the indicated severity level.
