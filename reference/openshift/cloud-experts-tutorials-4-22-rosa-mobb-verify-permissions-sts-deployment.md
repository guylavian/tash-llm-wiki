---
title: "Tutorial: Verifying permissions for a {product-title} STS deployment"
type: reference
domain: openshift
slug: cloud-experts-tutorials-4-22-rosa-mobb-verify-permissions-sts-deployment
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/cloud_experts_tutorials/rosa-mobb-verify-permissions-sts-deployment
version: 4.22
...
= Tutorial: Verifying permissions for a OpenShift Container Platform STS deployment

[role="_abstract"]
To proceed with the deployment of a OpenShift Container Platform cluster, an account must support the required roles and permissions. AWS Service Control Policies (SCPs) cannot block the API calls made by the installer or Operator roles.

Details about the IAM resources required for an STS-enabled installation of OpenShift Container Platform can be found here: About IAM resources for OpenShift Container Platform clusters that use STS.

This guide is validated for OpenShift Container Platform v4.11.X.
...
== Verifying OpenShift Container Platform permissions
To verify the permissions required for OpenShift Container Platform, we can run the script included in the following section without ever creating any AWS resources.
