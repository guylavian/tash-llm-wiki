...
type: reference
domain: openshift
slug: security-4-22-compliance-operator-release-notes
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/security/compliance-operator-release-notes
version: 4.22
family: security
...
* Before this release, when you created a `TailoredProfile` object that extended a base profile, you could only enable rules that were available in the XCCDF groups in the base profile. With this release, `TailoredProfile` objects can enable any rule available. For more information, see (CMP-4283).
...
* Red{nbsp}Hat recommends that customers upgrade to version 1.8.2 of Compliance Operator. For more information, see (CVE-2025-68973).
...
* Previously, Compliance Operator scans using rhcos4 profile would incorrectly return `NOT-APPLICABLE` scan results when using {op-system-first} 10 systems. With this release, scans using rhcos4 profiles return `COMPLIANT` and `NON-COMPLIANT` results. For more information, see (CMP-4034).
...
* This release removes the deprecated CIS 1.4.0, CIS 1.5.0, DISA STIG V1R1 and DISA STIG V2R1 profiles. The newer versions have replaced these obsolete profiles for customer use. For more information, see (CMP-3712).
...
// For more information, see (CMP-3172).
...
// For more information, see (OCPBUGS-60143.
...
// For more information, see (CMP-3589).
...
// For more information, see (CMP-3596).
...
// For more information, see (CMP-3599).
...
// For more information, see (CMP-3601).
...
// For more information, see (CMP-3630).
...
* After installing Cluster Logging Operator (CLO) 6.0, users found that the ComplianceCheckResult `ocp4-cis-audit-log-forwarding-enabled` was failing because there was a change in the APIversion of the `clusterlogforwarder` resource. Log collection and forwarding configurations are now specified under the new API, part of the observability.openshift.io API group. (OCPBUGS-43585)
...
* Previously, the rule `file-groupowner-ovs-conf-db-hugetlbf` would fail unexpectedly. With this release, the rule fails only when this is the needed result. (OCPBUGS-55190)
...
* Previously, the `ocp4-cis-scc-limit-container-allowed-capabilities` rule description contained ambiguous guidelines, leading to confusion among users. With this update, the rule description and actionable steps are clarified. (OCPBUGS-17828)
...
* Before this update, an issue with a `jq` filter caused errors with the `rhacs-operator-controller-manager` deployment during compliance checks. With this update, the `jq` filter expression is updated and the `rhacs-operator-controller-manager` deployment is exempt from compliance checks pertaining to container resource limits, eliminating false positive results. (OCPBUGS-19690)

* Before this update, `rhcos4-high` and `rhcos4-moderate` profiles checked values of an incorrectly titled configuration file. As a result, some scan checks could fail. With this update, the `rhcos4` profiles now check the correct configuration file and scans pass correctly. (OCPBUGS-31674)

* Previously, the `accessokenInactivityTimeoutSeconds` variable used in the `oauthclient-inactivity-timeout` rule was immutable, leading to a `FAIL` status when performing DISA STIG scans. With this update, proper enforcement of the `accessTokenInactivityTimeoutSeconds` variable operates correctly and a `PASS` status is now possible. (OCPBUGS-32551)
...
[id="compliance-operator-1-4-1-new-features-and-enhancements_{context}"]
== New features and enhancements
...
* Previously, the `STIG` profile was missing rules from CaC that fulfill requirements on the published `STIG` for OpenShift Container Platform. With this update, upon remediation, the cluster satisfies `STIG` requirements that can be remediated using Compliance Operator. (*OCPBUGS-26193*)
...
* Compliance Operator 1.3.0 now supports {ibm-power-name} and {ibm-z-name} for NIST 800-53 Moderate-Impact Baseline for OpenShift Container Platform platform and node profiles.
...
* The Compliance Operator can now be deployed on {hcp} using the software catalog by creating a `Subscription` file. For more information, see Installing the Compliance Operator on {hcp}.
...
// * Before this update, the `rhcos4-ensure-logrotate-activated` rule failed after auto-remediation was applied in OpenShift Container Platform 4.13 environments. With this update, rules with auto-remediation available have a `PASS` result after the auto-remedations are applied. (*OCPBUGS-10769*)
...
[id="compliance-operator-0-1-59-new-features-and-enhancements_{context}"]
== New features and enhancements
...
[id="compliance-operator-0-1-59-bug-fixes_{context}"]
== Bug fixes
...
[id="compliance-operator-0-1-53-bug-fixes_{context}"]
== Bug fixes
...
* Previously, group ownership for `/etc/openvswitch/conf.db` was incorrect on {ibm-z-name} architectures, resulting in `ocp4-cis-node-worker-file-groupowner-ovs-conf-db` check failures. Now, the check is marked `NOT-APPLICABLE` on {ibm-z-name} architecture systems. (*BZ#2072597*)
...
[id="compliance-operator-0-1-53-known-issue_{context}"]
== Known issue
...
[id="compliance-operator-0-1-52-new-features-and-enhancements_{context}"]
== New features and enhancements
...
[id="compliance-operator-0-1-52-bug-fixes_{context}"]
== Bug fixes
...
[id="compliance-operator-0-1-52-known-issue_{context}"]
== Known issue
...
[id="compliance-operator-0-1-49-new-features-and-enhancements_{context}"]
== New features and enhancements
...
[id="compliance-operator-0-1-49-bug-fixes_{context}"]
== Bug fixes
