...
type: reference
domain: openshift
slug: security-4-22-allowing-javascript-access-api-server
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/security/allowing-javascript-access-api-server
version: 4.22
family: security
...
where:
+
`additionalCORSAllowedOrigins`:: The hostname is specified as a Golang regular expression that matches against CORS headers from HTTP requests against the API server and OAuth server.
+
[NOTE]
====
