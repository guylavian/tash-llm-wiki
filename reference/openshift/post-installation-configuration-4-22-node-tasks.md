...
type: reference
domain: openshift
slug: post-installation-configuration-4-22-node-tasks
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/post_installation_configuration/node-tasks
version: 4.22
...
== Adding {op-system} compute machines to an OpenShift Container Platform cluster

You can add more {op-system-first} compute machines to your OpenShift Container Platform cluster on bare metal.
...
.. Edit the text file to add the `storage`, `filesystems`, and `systemd` stanzas for the partitions you want to use for the new node. You can specify any Ignition configuration parameters as needed.
+
[NOTE]
====
...
[IMPORTANT]
====
You can modify the control plane node size in a running OpenShift Container Platform  cluster for the following configurations only:
...
[NOTE]
====
In OpenShift Container Platform , half of a CPU core (500 millicore) is now reserved by the system by default compared to OpenShift Container Platform 3.11 and previous versions. The sizes are determined taking that into consideration.
====
...
[IMPORTANT]
====
OpenShift Container Platform supports the device plugin API, but the device plugin
...
[NOTE]
====
The OpenShift Container Platform cluster uses ephemeral storage to store information that does not have to persist after the cluster is destroyed.
====
...
In OpenShift Container Platform, because overcommitment of project-level resources is enabled by default, if required by your use case, you can disable overcommitment on projects that are not managed by Red Hat.
