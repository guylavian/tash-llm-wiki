---
title: "Creating {product-title} clusters using a custom AWS KMS encryption key"
type: reference
domain: openshift
slug: rosa-hcp-4-22-rosa-hcp-creating-cluster-with-aws-kms-key
tier: reference
source: https://docs.redhat.com/en/documentation/openshift_container_platform/4.22/html/rosa_hcp/rosa-hcp-creating-cluster-with-aws-kms-key
version: 4.22
family: rosa_hcp
...
= Creating OpenShift Container Platform clusters using a custom AWS KMS encryption key

[role="_abstract"]
By default, OpenShift Container Platform encrypts Amazon EBS volumes with an AWS managed KMS key. Use a custom KMS key to control key rotation, access policies, or meet specific compliance requirements.
...
= Creating a OpenShift Container Platform cluster using a custom AWS KMS key

[role="_abstract"]
You can create a OpenShift Container Platform cluster with a customer-provided KMS key that is used to encrypt either node root volumes, the etcd database, or both. A different KMS key ARN can be provided for each option.
